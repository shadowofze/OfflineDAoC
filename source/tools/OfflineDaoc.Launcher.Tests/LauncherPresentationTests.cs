...
public sealed class LauncherPresentationTests
{
    ...
    public void VersionIsManuallyPinnedAndRefreshRunsEveryFiveMinutes()
    {
        ...
        Assert.That(mainFormType.GetField("DisplayVersion", HiddenStatic)!.GetRawConstantValue(), Is.EqualTo("0.3.1"));
    ...
    public void CleanedLayoutKeepsInformationWithoutRedundantRealmTabOrRowSelectors()
    {
        ...
        Assert.That(controls.OfType<Label>().Any(label => label.Text.Contains("VERSION 0.3.1", StringComparison.Ordinal)), Is.True);
        Label version = controls.OfType<Label>().Single(label => label.Text == "VERSION 0.3.1");
