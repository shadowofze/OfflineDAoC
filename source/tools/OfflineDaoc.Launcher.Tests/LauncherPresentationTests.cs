...
public sealed class LauncherPresentationTests
{
    ...
    public void VersionIsManuallyPinnedAndRefreshRunsEveryFiveMinutes()
    {
        ...
        Assert.That(mainFormType.GetField("DisplayVersion", HiddenStatic)!.GetRawConstantValue(), Is.EqualTo("0.32b"));
    ...
    public void CleanedLayoutKeepsInformationWithoutRedundantRealmTabOrRowSelectors()
    {
        ...
        IReadOnlyList<Control> controls = Descendants(form).ToList();
            ...
            Assert.That(controls.OfType<Label>().Any(label => label.Text.Contains("VERSION 0.32b", StringComparison.Ordinal)), Is.True);
            Label version = controls.OfType<Label>().Single(label => label.Text == "VERSION 0.32b");
