...
namespace DOL.Database.Handlers
{
    public class SqliteObjectDatabase : SqlObjectDatabase
    {
        ...
        // Serialize every SQLite write transaction through one short-lived
        // in-process gate. WAL keeps reads concurrent; this only prevents server
        // writer threads from colliding and producing SQLITE_BUSY retry storms.
        protected override IEnumerable<bool> AddObjectImpl(DataTableHandler tableHandler, IEnumerable<DataObject> dataObjects)
        {
            lock (WriteSerializationLock)
                return base.AddObjectImpl(tableHandler, dataObjects).ToArray();
        }

        protected override IEnumerable<bool> SaveObjectImpl(DataTableHandler tableHandler, IEnumerable<DataObject> dataObjects)
        {
            lock (WriteSerializationLock)
                return base.SaveObjectImpl(tableHandler, dataObjects).ToArray();
        }
        ...
        {
            lock (WriteSerializationLock)
                return base.ExecuteNonQueryImpl(SQLCommand, parameters).ToArray();
        }
        ...
        {
            // The SQLite-specific last-insert-id path performs INSERT + id read
            // on one connection and must be part of the same writer sequence.
            if (retrieveLastInsertID)
            {
                lock (WriteSerializationLock)
                    return ExecuteScalarImplCore(SQLCommand, parameters, true);
            }

            return ExecuteScalarImplCore(SQLCommand, parameters, false);
        }

        private object[] ExecuteScalarImplCore(string SQLCommand, IEnumerable<IEnumerable<QueryParameter>> parameters, bool retrieveLastInsertID)
        {
            if (log.IsDebugEnabled)
