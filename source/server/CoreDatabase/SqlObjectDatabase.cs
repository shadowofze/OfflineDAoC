...
namespace DOL.Database
{
    ...
    public abstract class SqlObjectDatabase : ObjectDatabase
    {
        ...
        // SQLite permits many readers but only one writer.  The SQLite handler
        // uses this per-database gate for every write entry point so unrelated
        // autosave, inventory and exchange transactions queue in-process rather
        // than repeatedly entering SQLite's busy retry path.  Other database
        // handlers retain their existing concurrency because they do not take it.
        protected object WriteSerializationLock { get; } = new();
        ...
        /// <summary>
        /// Opt-in, all-or-nothing updates across already-persisted rows/tables.
        /// Unlike the ordinary per-table save batches, an exchange purchase must
        /// commit item ownership and both sides of its payment together. Relations
        /// are deliberately not traversed. Existing save paths are unchanged.
        /// </summary>
        public bool SaveObjectsAtomically(IEnumerable<DataObject> objects)
        {
            // DataObject equality compares ObjectId only, not its table. Wallet
            // row 2 and proceeds row 2 are different transaction participants.
            // Remove only the same object passed twice; never discard another
            // participant because it has an equal (or not-yet-assigned) ID.
            DataObject[] rows = objects?.Distinct<DataObject>(ReferenceEqualityComparer.Instance).ToArray();
            if (rows == null || rows.Length == 0 || rows.Any(row => row == null || !row.IsPersisted))
                return false;

            return SaveObjectsAtomicallyCore(rows);
        }

        protected virtual bool SaveObjectsAtomicallyCore(DataObject[] rows)
        {
            ...
            try
            {
                ...
                foreach (DataObject row in rows)
                {
                    DataTableHandler table = GetTableHandler(row.GetType());
                    if (table == null)
                        throw new DatabaseException("Atomic save: unregistered row type.");
                    ...
                    if (keys.Length == 0)
                        throw new DatabaseException("Atomic save: missing primary key.");
                    ...
                    if (changed.Length == 0)
                    {
                        ...
                        if (Convert.ToInt64(command.ExecuteScalar()) != 1)
                            throw new DatabaseException("Atomic save: a participating row no longer exists.");
                        continue;
                    }
                    string set = string.Join(", ", changed.Select(binding => $"`{binding.ColumnName}` = @{binding.ColumnName}"));
                    command.CommandText = $"UPDATE `{table.TableName}` SET {set} WHERE {where}";
                    ...
                    if (command.ExecuteNonQuery() != 1)
                        throw new DatabaseException("Atomic save: a participating row could not be updated.");
                }
                transaction.Commit();
                committed = true;
                foreach (DataObject row in rows)
            ...
            {
                try { log.Error(committed ? "Atomic exchange committed but cache refresh failed." : "Atomic exchange save failed; transaction rolled back.", exception); }
                catch { } // A logging failure must never bypass the caller's item/coin rollback.
                return committed;
            }
            finally
            {
                            ...
                            foreach (var parameter in parameters.Skip(current))
                            {
                                ...
                                catch (Exception ex)
                                {
                                    if (HandleSQLException(ex))
                                    {
                                        ...
                                        // SQLite can leave a failed prepared statement unusable.
                                        // Preserve results already written, and resume the remaining
                                        // rows with a fresh command/connection instead of poisoning
                                        // the rest of the batch (and losing its persistence flags).
                                        current++;
                                        repeat = true;
                                        break;
                                    }
