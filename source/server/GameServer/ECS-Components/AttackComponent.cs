...
namespace DOL.GS
{
    public class AttackComponent : IServiceObject
    {
        ...
        public eDamageType AttackDamageType(DbInventoryItem weapon, WeaponAction action)
        {
            ...
            if (playerOwner != null || owner is IGamePlayer || owner is CommanderPet)
            {
                if (weapon == null)
        ...
        public int AttackSpeed(DbInventoryItem mainWeapon, DbInventoryItem leftWeapon = null)
        {
            ...
            if (owner is GamePlayer or IGamePlayer)
            {
                ...
                int ownerQuickness = owner switch
                {
                    GamePlayer player => player.Quickness,
                    IGamePlayer playerBot => playerBot.Quickness,
                    _ => owner.GetModified(eProperty.Quickness),
                };
                int quickness = Math.Min(250, ownerQuickness); //250 soft cap on quickness
                ...
                if (bowWeapon)
                {
                    ...
                    speed *= (1.0 - (quickness - 60) * 0.002) * 0.01 * owner.GetModified(eProperty.MeleeSpeed);
        ...
        {
            double damage = owner switch
            {
                GamePlayer player => CalculatePlayerDamage(player, weapon, action),
                IGamePlayer playerBot => CalculatePlayerDamage(playerBot, weapon, action),
                _ => CalculateNpcDamage(weapon),
            };
        ...
        private double CalculatePlayerDamage(GamePlayer player, DbInventoryItem weapon, WeaponAction action)
        {
            ...
            return CalculatePlayerDamageCore(player.WeaponDamageWithoutQualityAndCondition(weapon), player.ActiveLeftWeapon, weapon, action);
        }
        ...
        private double CalculatePlayerDamage(IGamePlayer player, DbInventoryItem weapon, WeaponAction action)
        {
            ...
            return CalculatePlayerDamageCore(player.WeaponDamageWithoutQualityAndCondition(weapon), owner.ActiveLeftWeapon, weapon, action);
        }

        private double CalculatePlayerDamageCore(double weaponDamage, DbInventoryItem activeLeftWeapon, DbInventoryItem weapon, WeaponAction action)
        {
            double damage = weaponDamage * weapon.SPD_ABS * 0.1 * CalculateSlowWeaponDamageModifier(weapon);
        ...
        {
            if (owner is not GamePlayer && owner is not IGamePlayer || weapon == null)
                return 1.0;
        ...
        private void StartAttack()
        {
            if (owner is GamePlayer player)
            {
                ...
                {
                    PlayerLedPullCoordinator.LeaderEngaged(player, _startAttackTarget as GameLiving);
        ...
        private void NpcStartAttack()
        {
            ...
            bool autonomousBowPositioning = (BotRangedCombat.UsesAutonomousBowPositioning(npc) ||
                                            AutonomousDefensivePull.OwnsRangedPosition(npc)) &&
                                            npc.ActiveWeaponSlot is eActiveWeaponSlot.Distance;
            ...
            if (npc.ActiveWeaponSlot is eActiveWeaponSlot.Distance)
            {
                if (!npc.IsWithinRadius(_startAttackTarget, AttackRange - 30))
                {
                    if (attackAction == null || !attackAction.OnOutOfRangeOrNoLosRangedAttack())
                    {
                        ...
                        StopAttack();
                        if (autonomousBowPositioning)
                        {
                            int desired = BotRangedCombat.BowApproachDistance(npc.MeleeAttackRange, AttackRange);
                            npc.Follow(_startAttackTarget,
                                (short)Math.Clamp(desired, 160, short.MaxValue),
                                (short)Math.Clamp(Math.Max(desired + 200, npc.StickMaximumRange), 200, short.MaxValue));
                        }
                        else
            ...
            if (LivingStartAttack())
            {
                ...
                if (autonomousBowPositioning)
                {
                    // The bot brain already proved bow range before requesting
                    // the attack.  Face the target and let Aim reach release;
                    // the generic NPC melee-follow order is the animation-reset
                    // loop this autonomous-only branch prevents.
                    npc.StopFollowing();
                    if (npc.IsMoving)
                        npc.StopMoving();
                    npc.TurnTo(_startAttackTarget);
                }
                else if (_startAttackTarget != npc.FollowTarget)
                {
                    if (npc.IsMoving)
                        npc.StopMoving();
        ...
        {
            int levelDifference = (owner is GamePlayer or IGamePlayer ? owner.WeaponSpecLevel(weapon) : owner.Level) - targetLevel;
        ...
        public int CalculateSpec(DbInventoryItem weapon)
        {
            ...
            if ((owner is GamePlayer or IGamePlayer) && owner.Realm is eRealm.Albion && Properties.ENABLE_ALBION_ADVANCED_WEAPON_SPEC &&
        ...
        public (double, double) CalculateVarianceRange(GameLiving target, int spec)
        {
            ...
            else if (owner is IGamePlayer botOwner)
            {
                if (botOwner.SpecLock > 0)
                    return (botOwner.SpecLock, botOwner.SpecLock);
        ...
        public double GetMissChance(WeaponAction action, AttackData ad, AttackData lastAD, DbInventoryItem weapon)
        {
            ...
            if (((owner is GamePlayer or IGamePlayer) && owner.IsSitting) || action.RangedAttackType is eRangedAttackType.Volley)
               return 0;
        ...
        public bool CanUseLefthandedWeapon
        {
            get
            {
                if (owner is GamePlayer playerOwner)
                    ...
                    return botOwner.CharacterClass.CanUseLefthandedWeapon;
                else if (owner is GameNPC)
                    return true;
        ...
        public int CalculateLeftHandSwingCount(DbInventoryItem mainWeapon, DbInventoryItem leftWeapon)
        {
            ...
            if (owner is GameNPC npcOwner && owner is not IGamePlayer)
            {
            ...
            GamePlayer playerOwner = owner as GamePlayer;
            if (owner is not GamePlayer && owner is not IGamePlayer ||
                (eObjectType)leftWeapon.Object_Type is eObjectType.Shield || mainWeapon == null)
                return 0;
