...
namespace DOL.GS
{
    public class RangeAttackComponent
    {
        ...
        private DbInventoryItem _botAmmo;
        ...
        public DbInventoryItem UpdateAmmo(DbInventoryItem weapon)
        {
            ...
            if (_owner is GameBot && BotRangedCombat.IsUsableWeapon(weapon))
            {
                if ((eObjectType)weapon.Object_Type == eObjectType.Thrown)
                    return Ammo = weapon; // Reusable weapon; never consume its stack.
                eObjectType type = (eObjectType)weapon.Object_Type == eObjectType.Crossbow ? eObjectType.Bolt : eObjectType.Arrow;
                if (_botAmmo == null || _botAmmo.Object_Type != (int)type || _botAmmo.Type_Damage != weapon.Type_Damage)
                    _botAmmo = GameInventoryItem.Create(new DbItemTemplate
                    {
                        Name = "bot standard ammunition", Object_Type = (int)type,
                        Type_Damage = weapon.Type_Damage, SPD_ABS = 0x15,
                        // Neutral damage/range/accuracy. Never inserted into an
                        // inventory or database; cached until weapon type changes.
                        IsDropable = false, IsTradable = false, MaxCount = 1,
                    });
                return Ammo = _botAmmo;
            }
        ...
        public void RemoveEnduranceAndAmmoOnShot()
        {
            ...
            if (_owner is not GameBot && (arrowRecoveryChance == 0 || Util.Chance(100 - arrowRecoveryChance)))
                _owner.Inventory.RemoveCountFromStack(Ammo, 1);

            _owner.Endurance -= ShotEnduranceCost;
        }

        public int ShotEnduranceCost => RangedAttackType switch
        {
            eRangedAttackType.Critical => CRITICAL_SHOT_ENDURANCE_COST,
            eRangedAttackType.RapidFire when _owner.GetAbilityLevel(Abilities.RapidFire) == 2 => (int)Math.Ceiling(DEFAULT_ENDURANCE_COST / 2.0),
            eRangedAttackType.Volley => VOLLEY_ENDURANCE_COST,
            _ => DEFAULT_ENDURANCE_COST,
        };
    }
}
