	...
	public class WhoCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		public void OnCommand(GameClient client, string[] args)
		{
			...
			foreach (GameBot bot in worldBots)
			{
				...
				resultCount++;
				if (resultMessages.Count < MAX_LIST_SIZE && resultCount >= listStart)
					resultMessages.Add(resultCount + ") " + FormatBotLine(bot, client));
			}
		...
		private static string FormatBotLine(GameBot bot, GameClient source)
		{
			StringBuilder result = new StringBuilder(bot.Name, 100);
			result.Append(" the Level ").Append(bot.Level).Append(' ').Append(bot.ClassName);
			if (bot.CurrentZone != null && GameServer.Instance.Configuration.ServerType != EGameServerType.GST_PvP)
			{
				result.Append(" in ");
				if (source.Account.PrivLevel == (uint)ePrivLevel.Player && bot.CurrentZone.IsRvR && bot.CurrentRegionID is 1 or 100 or 200)
					result.Append("the Frontiers");
				else
					result.Append(bot.CurrentZone.Description);
			}
			result.Append(" [Playerbot]");
			return result.ToString();
		}

		private static bool ApplyBotFilter(string[] args, GameBot bot)
		{
			if (args.Length < 2 || args[1].Equals("all", StringComparison.OrdinalIgnoreCase))
				return true;
			string first = args[1];
			string key = first.ToLowerInvariant();
			if (first.Equals("nogroup", StringComparison.OrdinalIgnoreCase))
				return bot.Group == null;
			if (first.Equals("frontiers", StringComparison.OrdinalIgnoreCase))
				return bot.CurrentZone?.IsRvR == true;
			if (key is "help" or "staff" or "gm" or "admin" or "cg" or "bg" or "adv" or "rp" or "hc" or "hardcore")
				return false;
			if (int.TryParse(first, out int minimum))
			{
				if (args.Length > 2 && int.TryParse(args[2], out int maximum))
					return bot.Level >= Math.Min(minimum, maximum) && bot.Level <= Math.Max(minimum, maximum);
				return bot.Level == minimum;
			}

			return args.Skip(1).All(filter =>
				bot.Name.StartsWith(filter, StringComparison.OrdinalIgnoreCase) ||
				(bot.ClassName?.StartsWith(filter, StringComparison.OrdinalIgnoreCase) ?? false) ||
				(bot.CurrentZone?.Description?.Contains(filter, StringComparison.OrdinalIgnoreCase) ?? false));
		}
