	...
	public class RangeSpellHandler : PrimerSpellHandler
	{
		...
		public override int PowerCost(GameLiving target)
		{
			...
			if (basepower < 0)
			{
				...
				else if (Caster is GameBot bot && bot.CharacterClass.ManaStat != eStat.UNDEFINED)
				{
					basepower = bot.CalculateMaxMana(bot.Level, bot.GetBaseStat(bot.CharacterClass.ManaStat)) * basepower * -0.01;
				}
				else
				{
