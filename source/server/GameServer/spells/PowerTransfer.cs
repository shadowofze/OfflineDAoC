...
namespace DOL.GS.Spells
{
	...
	class PowerTransfer : SpellHandler
	{
		...
		public override bool CheckBeginCast(GameLiving selectedTarget)
		{
			...
			{
				MessageToLiving(owner, "You cannot transfer power to yourself!", eChatType.CT_SpellResisted);
				return false;
			}
		...
		public override void OnDirectEffect(GameLiving target)
		{
			...
			if (powerHealed <= 0)
			{
				...
				MessageToLiving(owner, String.Format("{0} is at full power already!",
					target.Name), eChatType.CT_SpellResisted);
			}
			else
			{
				SendEffectAnimation(target, 0, false, 1);
				MessageToLiving(owner, String.Format("You transfer {0} power to {1}!",
					powerHealed, target.Name), eChatType.CT_Spell);
		...
		{
			if (Caster is GamePlayer or GameBot)
				return Caster;
