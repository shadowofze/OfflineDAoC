...
namespace DOL.GS.Spells
{
	...
	public class SpellHandler : ISpellHandler
	{
		...
		public bool IsQuickCasting => Caster is not GameBot && _quickcast != null;
		...
		public virtual bool CheckBeginCast(GameLiving selectedTarget, bool quiet)
		{
			...
			switch (Spell.Target)
			{
				...
				case eSpellTarget.PET:
				{
					...
					IControlledBrain selectedPetBrain = (Target as GameNPC)?.Brain as IControlledBrain;
					bool selectedPetOwnedByCaster = Spell.IsBuff && !Spell.IsHealing
						? Target is GameNPC selectedBuffPet &&
						  ControlledPetBuffScope.IsOwnedPetTreeMember(Caster, selectedBuffPet)
						: selectedPetBrain != null &&
						  (selectedPetBrain.GetLivingOwner() == Caster || selectedPetBrain.GetPlayerOwner() == Caster);
					bool selectedPartyPet = Spell.IsHealing && Caster is GameBot companion &&
						TemporaryCompanionPetHealing.IsPartyControlledPet(companion, Target, out _);
					if (!selectedPetOwnedByCaster && !selectedPartyPet && Caster.ControlledBrain?.Body != null)
			...
			else if (Caster is GameNPC npcOwner)
			{
				...
				// Instrument songs are legal while moving. A bot is an NPC-backed
				// player actor, so do not apply the generic NPC cast-stop rule to a
				// song that the player casting path also permits while in motion.
				if (!Spell.IsInstantCast && Spell.InstrumentRequirement == 0 && !Spell.MoveCast)
				{
			...
			CancelFocusSpells();
			_quickcast = m_caster is GameBot ? null :
				EffectListService.GetAbilityEffectOnTarget(m_caster, eEffect.QuickCast) as QuickCastECSGameEffect;
			...
			else if (m_caster.IsSitting && !BotSongTwistPolicy.IsMobileSong(m_caster, m_spell))
			{
			...
			switch (Spell.Target)
			{
				case eSpellTarget.PET:
				{
					IControlledBrain petBrain = (Target as GameNPC)?.Brain as IControlledBrain;
					bool petOwnedByCaster = Spell.IsBuff && !Spell.IsHealing
						? Target is GameNPC buffPet &&
						  ControlledPetBuffScope.IsOwnedPetTreeMember(Caster, buffPet)
						: petBrain != null &&
						  (petBrain.GetLivingOwner() == Caster || petBrain.GetPlayerOwner() == Caster);
					bool partyPet = Spell.IsHealing && Caster is GameBot companion &&
						TemporaryCompanionPetHealing.IsPartyControlledPet(companion, Target, out _);
					if (Target == null || (!petOwnedByCaster && !partyPet))
					{
						if (!quiet)
			...
			if (CostType == SpellCostType.Power && Spell.Power != 0 &&
				m_caster.Mana < PowerCost(Target) &&
				!IsQuickCasting &&
				Spell.SpellType is not eSpellType.Archery)
			{
				if (!quiet && Caster is not GameBot)
		...
		public bool CheckConcentrationCost(bool quiet)
		{
			...
			{
				if (!quiet && m_caster is not GameBot)
					MessageToCaster($"This spell requires {m_spell.Concentration} concentration points to cast!", eChatType.CT_SpellResisted);

				return false;
			}

			if (m_caster is not GameBot && m_caster.effectListComponent.GetConcentrationEffects().Count >= MAX_CONC_SPELLS)
			{
				if (!quiet)
		...
		public virtual bool CheckEndCast(GameLiving target)
		{
			...
			if (IsSummoningSpell && Caster.CurrentRegion.IsCapitalCity &&
				!AutonomousPetSupport.CanSummonInCapital(Caster))
			{
				// Message: You can't summon here!
				if (verbose && Caster is GamePlayer playerCaster)
					ChatUtil.SendErrorMessage(playerCaster, "GamePlayer.CastEnd.Fail.BadRegion", null);

				return false;
			}
			
			if (target != null && Caster != target && Caster is GameNPC casterNPC && Caster is not NecromancerPet)
			...
			else if (m_caster.IsSitting && !BotSongTwistPolicy.IsMobileSong(m_caster, m_spell)) // Bot vocal chants also allow sitting.
			{
				// Purge can be cast while sitting but only if player has negative effect that doesn't allow standing up (like stun or mez).
				if (verbose)
					MessageToCaster("You can't cast while sitting!", eChatType.CT_SpellResisted);
			...
			if (CostType == SpellCostType.Power && m_caster.Mana <= 0 && Spell.Power > 0 &&
				Spell.SpellType is not eSpellType.Archery)
			{
				if (verbose && Caster is not GameBot)
					MessageToCaster("You have exhausted all of your power and cannot cast spells!", eChatType.CT_SpellResisted);

				return false;
			}

			if (CostType == SpellCostType.Power && (Spell.Power > 0 || Caster is GameBot && Spell.Power < 0) &&
				m_caster.Mana < PowerCost(target) && !IsQuickCasting && Spell.SpellType is not eSpellType.Archery)
			{
				if (verbose && Caster is not GameBot)
					MessageToCaster("You don't have enough power to cast that!", eChatType.CT_SpellResisted);
		...
		public virtual bool CheckDuringCast(GameLiving target, bool quiet)
		{
			...
			{
				// Instrument songs remain legal while moving throughout the cast,
				// not just at its start. Do not stop a Bard/Minstrel's follow route.
				if (Spell.CastTime > 0 && Spell.InstrumentRequirement == 0 && !Spell.MoveCast)
				{
					if (npcOwner.IsMoving)
		...
		public void Tick()
		{
			switch (CastState)
			{
                ...
                {
                    // Exhausted power, dispels and pulse replacement can end the
                    // effect without calling CancelFocusSpells. NPCs otherwise
                    // retain this handler forever and queue every future cast
                    // behind it. Let the existing cleanup promote that queue.
                    // Do not interrupt a live channel, a starting pulse, a
                    // non-pulsing focus, or another spell that replaced us.
                    if (Spell.IsPulsing && PulseEffect?.IsActive != true && PulseEffect?.IsStarting != true)
                    {
                        if (!ReferenceEquals(Caster.castingComponent?.SpellHandler, this))
                            return;

                        CastState = eCastState.Cleanup;
                        if (Caster is GameBot bot && log.IsInfoEnabled)
                            log.Info($"AUTONOMOUS_FOCUS_RELEASE bot=\"{bot.Name}\" id={bot.DatabaseID} level={bot.Level} " +
                                $"spellId={Spell.ID} reason=inactive-pulse");
                    }
		...
		public virtual int PowerCost(GameLiving target)
		{
			...
			if (Caster is GameBot botCaster)
				return BotSpellPower.Cost(botCaster, Spell, SpellLine);
		...
		protected virtual void InterruptCasting(bool isMoving)
		{
			...
			m_interrupted = true;
			// An instant cast executes beside the primary cast. Its failed
			// range/power/interrupt check must not cancel a bot's unrelated
			// in-flight spell (or send an interrupt animation for that spell).
			if (Caster is not GameBot || Caster.castingComponent.SpellHandler == null ||
				ReferenceEquals(Caster.castingComponent.SpellHandler, this))
		...
		public virtual List<GameLiving> SelectTargets(GameObject castTarget)
		{
			...
			switch (modifiedTarget)
			{
				...
				case eSpellTarget.PET:
				{
					bool EligiblePet(GameNPC candidate) => Spell.IsBuff && !Spell.IsHealing
						? ControlledPetBuffScope.IsOwnedPetTreeMember(Caster, candidate)
						: Caster.IsControlledNPC(candidate);
					...
					if (modifiedRadius > 0 && Spell.Range == 0)
					{
						...
						{
							if (EligiblePet(npcInRadius))
								list.Add(npcInRadius);
						}

						TemporaryCompanionPetHealing.AdjustHealingTargets(m_caster, Spell, list, false);
						return list;
					}
					...
					if (pet != null)
					{
						...
						{
							if (npcInRadius == pet || !EligiblePet(npcInRadius) || npcInRadius.Brain is BomberBrain)
								continue;
			...
			TemporaryCompanionPetHealing.AdjustHealingTargets(m_caster, Spell, list,
				modifiedTarget is eSpellTarget.GROUP);
			return list;
		}
		...
		public virtual List<GameLiving> GetGroupAndPets(Spell spell)
		{
			...
			// Bonedancer subpets are controlled by the commander rather than
			// directly by the caster. Resolve the root Bonedancer group so Realm
			// broad support buffs such as Bogdar's Boon and Renew Flesh expand to
			// the actual companion/playerbot party instead of only the subpet.
			if (groupMembers == null &&
				Caster is BdSubPet { Brain: IControlledBrain subPetBrain } &&
				subPetBrain.Owner is CommanderPet { Brain: IControlledBrain commanderBrain })
			{
				GameLiving bonedancer = commanderBrain.Owner;
				groupMembers = bonedancer?.Group?.GetMembersInTheGroup();
				if (groupMembers == null && bonedancer != null)
					groupMembers = new List<GameLiving> { bonedancer };
			}
			...
			TemporaryCompanionPetHealing.AdjustHealingTargets(Caster, spell, livingsInRange, true);
		...
		public virtual bool StartSpell(GameLiving target)
		{
			...
			// Bonedancer support minions explicitly select one finite, in-range
			// target at a time. Do not re-expand their Realm buffs here: doing so
			// gives Bogdar's Boon different semantics from Bladeturn/Renew Flesh
			// and makes the brain unable to tell which member still needs a cast.
			// The Patroller also limits Shards of Bone to its commander tree.
			bool isBonedancerSupportMinion = Caster is BdSubPet
			{
				Brain: BdBufferBrain or BdHealerBrain
			};

			if (!isBonedancerSupportMinion
				&& Spell.Target is eSpellTarget.REALM
...
// 			}
		...
		public virtual void CalculateDamageVariance(GameLiving target, out double min, out double max)
		{
			...
			switch (m_spellLine.KeyName)
			{
				...
				default:
				{
					...
					if (m_caster is NecromancerPet necromancerPet && necromancerPet.Brain is IControlledBrain brain)
						casterToUse = brain.Owner is GameBot botOwner ? botOwner : brain.GetLivingOwner() ?? m_caster;
		...
		public virtual double CalculateDamageBase(GameLiving target)
		{
			...
			else if (modifiedCaster is IGamePlayer botCaster)
			{
				switch ((eCharacterClass) botCaster.CharacterClass.ID)
				{
					case eCharacterClass.MaulerAlb:
					case eCharacterClass.MaulerMid:
					case eCharacterClass.MaulerHib:
					...
					case eCharacterClass.Nightshade:
					{
						stat = botCaster.GetModified(eProperty.Strength) + botCaster.AbilityBonus[eProperty.Acuity];
						break;
					}
					default:
					{
						if (botCaster.CharacterClass.ManaStat is not eStat.UNDEFINED)
							stat = botCaster.GetModified((eProperty) botCaster.CharacterClass.ManaStat);

						spec = modifiedCaster.GetModifiedFromItems(SkillBase.SpecToSkill(m_spellLine.Spec));
						break;
					}
				}
			}
