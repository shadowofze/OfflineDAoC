...
namespace DOL.GS.Spells
{
	...
	public class SummonTheurgistPet : SummonSpellHandler
	{
		...
		{
			if (!PetLimitAllowsAnother(Caster.PetCount, ServerProperties.Properties.THEURGIST_PET_CAP))
			{
		...
		public static bool PetLimitAllowsAnother(int currentCount, int configuredCap)
		{
			// The numerical cap was added after the 1.65 era. Zero therefore means
			// no artificial cap; cast time, power cost and pet duration remain the
			// authoritative natural limits.
			return configuredCap <= 0 || currentCount < configuredCap;
		}
