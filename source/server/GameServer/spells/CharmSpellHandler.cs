...
namespace DOL.GS.Spells
{
    ...
    {
        private GameNPC _generatedPlayerCandidate;
        ...
        {
            // Only human player rank charms open a menu. Bot handlers never
            // enter this path. Selection resumes the normal cast/pet lifecycle.
            if (Caster is GamePlayer player && PlayerCharmSelectionMenu.Supports(player, m_spell))
            {
                DOL.Database.DbMob choice = PlayerCharmSelectionMenu.TakeChoice(player, m_spell);
                if (choice == null)
                {
                    PlayerCharmSelectionMenu.Open(player, m_spell, SpellLine);
                    return false;
                }
                if (!AutonomousPetSupport.TryPreparePlayerGeneratedCharm(
                        player, m_spell, choice, out GameNPC candidate, out Spell permanentCharm))
                {
                    MessageToCaster("Your selected creature could not be summoned. Cast the charm again to choose.", eChatType.CT_System);
                    return false;
                }
                _generatedPlayerCandidate = candidate;
                m_spell = permanentCharm;
                selectedTarget = candidate;
                player.TargetObject = candidate;
                player.TargetInView = true;
            }

            bool accepted = base.CheckBeginCast(selectedTarget);
            if (!accepted && _generatedPlayerCandidate != null)
            {
                AutonomousPetSupport.AbandonPendingSyntheticCharm(Caster, _generatedPlayerCandidate);
                _generatedPlayerCandidate = null;
            }
            return accepted;
        }
        ...
        public override bool CheckEndCast(GameLiving selectedTarget)
        {
            if (Caster is GameBot botCaster)
                return CheckBotEndCast(botCaster, selectedTarget);
            ...
            if (Caster.ControlledBrain == null)
            {
                ...
                if (casterPlayer.CharacterClass.ID is (int)eCharacterClass.Minstrel or (int)eCharacterClass.Mentalist)
                {
                    ...
                    if (charmMob != _generatedPlayerCandidate &&
                        charmMob.Level > casterPlayer.GetModifiedSpecLevel(m_spellLine.Spec) * 1.1)
                    {
                        // Message: {0} is too strong for you to charm!
                        MessageToCaster(LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.TooStrong", charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                        return false;
                ...
                if (casterPlayer.CharacterClass.ID is (int)eCharacterClass.Hunter or (int)eCharacterClass.Sorcerer)
                {
                    ...
                    if (charmMob != _generatedPlayerCandidate && charmMob.Level > Caster.Level)
                    {
                        // Message: {0} is too strong for you to charm!
                        MessageToCaster(LanguageMgr.GetTranslation(casterPlayer.Client, "CharmSpell.EndCast.Fail.TooStrong", charmMob.GetName(0, true)), eChatType.CT_SpellResisted);
                        return false;
        ...
        private bool CheckBotEndCast(GameBot casterBot, GameLiving selectedTarget)
        {
            if (selectedTarget is not GameNPC charmMob || !base.CheckEndCast(charmMob))
                return false;
            if (casterBot.ControlledBrain != null && casterBot.ControlledBrain != charmMob.Brain)
                return false;
            if (charmMob.Brain is IControlledBrain controlled && controlled.Owner != casterBot)
                return false;
            if (!charmMob.IsAlive || charmMob.ObjectState is not GameObject.eObjectState.Active ||
                charmMob.CurrentRegion != casterBot.CurrentRegion || charmMob.Realm != eRealm.None ||
                charmMob.InCombat || charmMob.BodyType is < 1 or > 11)
                return false;
            ...
            int intendedLevel = syntheticCandidate
                ...
                : Math.Max(1, casterBot.Level - 2);
            if (intendedLevel <= 0 || !AutonomousPetSupport.IsCharmBodyTypeAllowed(Spell, charmMob.BodyType))
                return false;

            return charmMob.Level == intendedLevel && charmMob.Level <= casterBot.Level &&
                   (Spell.Value <= 0 || charmMob.Level <= Spell.Value);
        }
        ...
        public override void ApplyEffectOnTarget(GameLiving target)
        {
            ...
            if (playerCaster != null)
            {
                ...
                if (target == _generatedPlayerCandidate)
                    resistChance = 0;
                else if (Spell.Pulse != 0) // not permanent
                {
                    /*
                ...
                if (target.IsWithinRadius(playerCaster, 2000))
                {
                    ...
                    if (spellResistChance > 0 && resistResult <= spellResistChance)
                    {
