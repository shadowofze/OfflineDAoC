...
namespace DOL.GS
{
	...
	public class LootGeneratorMobTemplate : LootGeneratorBase
	{
		...
		public override LootList GenerateLoot(GameNPC mob, GameObject killer)
		{
			...
			try
			{
				GameLiving lootOwner = ResolveLootOwner(killer);
				GamePlayer player = lootOwner as GamePlayer;
				if (lootOwner == null)
					return loot;
				...
				if (!m_mobXLootTemplates.TryGetValue(mob.Name.ToLower(), out killedMobXLootTemplates))
				{
					...
					if (m_lootTemplates.TryGetValue(mob.Name.ToLower(), out lootTemplatesToDrop))
					{
						foreach (DbDropTemplateXItemTemplate lootTemplate in lootTemplatesToDrop.Values)
						{
							...
							if (CanReceiveRealmDrop(lootOwner, drop))
							{
				...
				else
				{
					...
					{
						loot = GenerateLootFromMobXLootTemplates(mobXLootTemplate, lootTemplatesToDrop, loot, lootOwner);
		...
		private static LootList GenerateLootFromMobXLootTemplates(DbMobDropTemplate mobXLootTemplates, List<DbDropTemplateXItemTemplate> lootTemplates, LootList lootList, GameLiving lootOwner)
		{
			if (mobXLootTemplates == null || lootTemplates == null || lootOwner == null)
				return lootList;
