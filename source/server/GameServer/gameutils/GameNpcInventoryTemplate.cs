	...
	public class GameNpcInventoryTemplate : GameLivingInventory
	{
		...
		public static bool Init()
		{
			try
			{
				...
				lock (_missingNpcEquipmentCacheLock)
					m_missingNpcEquipmentCache.Clear();
		...
		private static readonly Dictionary<string, long> m_missingNpcEquipmentCache = new(StringComparer.Ordinal);
		...
		public override bool LoadFromDatabase(string templateId)
		{
			...
			List<DbNpcEquipment> items = null;
			if (m_npcEquipmentCache != null)
				m_npcEquipmentCache.TryGetValue(templateId, out items);
			if (items == null)
			{
				lock (_missingNpcEquipmentCacheLock)
				{
					long now = Environment.TickCount64;
					if (m_missingNpcEquipmentCache.TryGetValue(templateId, out long expires) && now < expires)
						return false;
					m_missingNpcEquipmentCache.Remove(templateId);
				}
				...
				{
					lock (_missingNpcEquipmentCacheLock)
						m_missingNpcEquipmentCache[templateId] = Environment.TickCount64 + MissingTemplateCacheMilliseconds;
				}
			}
