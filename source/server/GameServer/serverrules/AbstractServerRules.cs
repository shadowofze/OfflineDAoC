...
namespace DOL.GS.ServerRules
{
    public abstract class AbstractServerRules : IServerRules
    {
        ...
        public virtual bool IsAllowedToAttack(GameLiving attacker, GameLiving defender, bool quiet)
        {
            ...
            // Apply shade protection before the NPC/bot fast paths as well.
            // Attackers must fight the necromancer's pet, not the immune shade.
            if (IsProtectedNecromancerShade(defender))
            {
                if (!quiet)
        ...
        public static bool IsProtectedNecromancerShade(GameLiving defender) =>
            defender?.ControlledBrain?.Body is NecromancerPet { IsAlive: true, ObjectState: GameObject.eObjectState.Active };
        ...
        public virtual bool CheckAbilityToUseItem(GameLiving living, DbItemTemplate item)
        {
            ...
            IGamePlayer playerLike = living as IGamePlayer;
            ...
            // classes restriction.
            if (playerLike != null && !string.IsNullOrEmpty(item.AllowedClasses))
            {
                if (!Util.SplitCSV(item.AllowedClasses, true).Contains(playerLike.CharacterClass.ID.ToString()))
                    return false;
            }
            ...
            if (item.Object_Type >= (int)eObjectType._FirstArmor && item.Object_Type <= (int)eObjectType._LastArmor)
            {
                ...
                {
                    switch (living.Realm) // Choose based on the player-like living rather than item region.
                    {
        ...
        {
            // Persistent autonomous bots represent real realm opponents in RvR.
            // Award their RP through the player-equivalent path, then stop here:
            // they must not also be paid as an NPC. Controlled pets and temporary
            ...
            if (!ProcessXpGainers(killedNpc,
                ...
                out ItemOwnerTotalDamagePair mostDamagingBot,
            ...
            // Support bots earn the same kill credit without duplicating XP awards.
            var expeditionOwner = AutonomousRealmRaid.LootOwner(killedNpc, botCountAndDamage.Keys) as RealmRaidLootOwner;
            var expeditionMembers = expeditionOwner?.Members.ToHashSet();
            foreach (var pair in botCountAndDamage)
            {
                GameBot bot = pair.Key;
                AwardBotOnNpcKill(bot, totalDamage, killedNpc, botCountAndDamage, groupCountAndDamage,
                    expeditionMembers?.Contains(bot) != true);
            }
            if (expeditionMembers != null)
                foreach (GameBot bot in expeditionMembers)
                {
                    AutonomousObjectiveAssignments.RecordPveKill(bot);
                    AutonomousGoalDiagnostics.Reward(bot, killedNpc, 0);
                }
            ...
            if (killedNpc.CanDropLoot)
            {
                ...
                if (mostDamagingPlayer?.Owner != null)
                    itemOwners.Add(mostDamagingPlayer);

                if (mostDamagingBot?.Owner != null)
                    itemOwners.Add(mostDamagingBot);

                if (expeditionOwner != null)
                {
                    itemOwners.Clear();
                    // Autonomous expeditions share actual drops across the roster,
                    // regardless of party or which actor delivered the last hit.
                    itemOwners.Add(new ItemOwnerTotalDamagePair(expeditionOwner, totalDamage));
                }
                ...
                out ItemOwnerTotalDamagePair mostDamagingBattlegroup)
            {
                ...
                // Process real participants first. Temporary /spawn helpers are
                // deliberately handled afterward so their damage can be
                // credited to their owner without incrementing the group size.
                             ...
                             pair.Key is not GameBot { IsTemporaryGroupHelper: true }))
                {
                    ...
                    if (pair.Key.ObjectState is not GameObject.eObjectState.Active ||
                        !pair.Key.IsWithinRadius(killedNpc, WorldMgr.MAX_EXPFORKILL_DISTANCE))
                        continue;

                    if (pair.Key is GameBot bot)
                    {
                        if (!bot.IsAutonomousWorldBot)
                            continue;

                        ProcessBotDamage(bot, pair.Value, botCountAndDamage, mostDamagingBot);

                        Group botGroup = bot.Group;
                        if (botGroup != null)
                        {
                            groupCountAndDamage ??= new();
                            mostDamagingGroup ??= new();
                            ProcessDamage(bot, pair.Value, botGroup, mostDamagingGroup, groupCountAndDamage);
                        }

                        continue;
                    }

                    // Remaining reward participants are normal client players.
                ...
                foreach (var pair in creditedGainers.Where(pair =>
                             pair.Key is GameBot { IsTemporaryGroupHelper: true }))
                {
                    GameBot helper = (GameBot)pair.Key;
                    GamePlayer owner = helper.Owner;
                    if (owner == null || owner.ObjectState is not GameObject.eObjectState.Active ||
                    ...
                    if (group != null)
                    {
                        groupCountAndDamage ??= new();
                        mostDamagingGroup ??= new();
                        ProcessDamage(owner, pair.Value, group, mostDamagingGroup,
                            groupCountAndDamage, false);
                    }
                }
                    ...
                    ItemOwnerTotalDamagePair mostDamagingBot)
                {
                    ...
                    {
                        entityDamage[bot] = new(1, damage, bot);
                        total = damage;
                    }
                ...
                static void ProcessDamage<T>(GameLiving participant, double damage, T entity,
                    ItemOwnerTotalDamagePair mostDamagingEntity,
                    Dictionary<T, EntityCountTotalDamagePair> entityDamage,
                    bool incrementCount = true) where T : class, IGameStaticItemOwner
                {
                    double totalDamage;
                    ...
                    {
                        totalDamage = damage;
                        entityDamage[entity] = new(incrementCount ? 1 : 0, totalDamage, participant);
                    }
            ...
            Dictionary<BattleGroup, EntityCountTotalDamagePair> battlegroupCountAndDamage)
        {
            ...
            long CalculateNpcExperienceModifiedByGroupOrBattlegroup(EntityCountTotalDamagePair entityCountTotalDamagePair)
            {
                ...
                GameLiving highestLevelPlayer = entityCountTotalDamagePair.HighestLevelPlayer;
        ...
        private static void AwardBotOnNpcKill(GameBot botToAward,
            double npcTotalDamageReceived,
            GameNPC killedNpc,
            ...
            Dictionary<Group, EntityCountTotalDamagePair> groupCountAndDamage, bool recordKillCredit = true)
        {
            ...
            if (botToAward.Group != null)
            {
                ...
                baseXpReward = CalculateGroupExperience(contribution);
            }
            else
            {
                if (!botCountAndDamage.TryGetValue(botToAward, out contribution))
                    return;

                baseXpReward = killedNpc.ExperienceValue;
            }
            ...
            double fullCampBonus = killedNpc.CurrentZone.IsDungeon
                ? Properties.MAX_DUNGEON_CAMP_BONUS
                : Properties.MAX_CAMP_BONUS;
            double campBonusPercent;
            if (GameLoop.GameLoopTime - killedNpc.SpawnTick > 1800000)
            {
                campBonusPercent = fullCampBonus;
                killedNpc.CampBonus = 0.98;
            }
            else
            {
                campBonusPercent = fullCampBonus * killedNpc.CampBonus;
            }

            long campBonus = (long)(baseXpReward * Math.Max(0, campBonusPercent));
            long groupBonus = 0;
            if (botToAward.Group != null && groupCountAndDamage != null &&
                groupCountAndDamage.TryGetValue(botToAward.Group, out EntityCountTotalDamagePair groupContribution))
            {
                groupBonus = (long)(baseXpReward * Math.Max(0, groupContribution.Count - 1) * 0.125);
            }

            long guildBonus = botToAward.Guild?.BonusType is Guild.eBonusType.Experience
                ? (long)(baseXpReward * Properties.GUILD_BUFF_XP * 0.01)
                : 0;
            long bafBonus = killedNpc.Brain is StandardMobBrain brain
                ? (long)(baseXpReward * brain.BafAddCount * 0.075)
                : 0;

            long diagnosticXpBefore = botToAward.Experience;
            botToAward.GainExperience(new GainedExperienceEventArgs(
                baseXpReward,
                campBonus,
                groupBonus,
                guildBonus,
            ...
            long CalculateGroupExperience(EntityCountTotalDamagePair groupContribution)
            {
                int memberCount = groupContribution.Count;
                if (memberCount <= 1)
                    return killedNpc.ExperienceValue;

                GameLiving highestLevelMember = groupContribution.HighestLevelPlayer;
                ConColor highestCon = ConLevels.GetConColor(highestLevelMember.GetConLevel(killedNpc));
                if (highestCon is ConColor.GREY)
                    return 0;

                ConColor challengeThreshold = memberCount switch
                {
                    >= 8 => ConColor.RED,
                    >= 4 => ConColor.ORANGE,
                    _ => ConColor.YELLOW,
                ...
                if (highestCon >= challengeThreshold || highestLevelMember == botToAward ||
                    ConLevels.GetConColor(botToAward.GetConLevel(killedNpc)) <= highestCon)
                {
                    return (long)Math.Ceiling((double)killedNpc.ExperienceValue / memberCount);
                }

                int equivalentLevel = 0;
                if (highestCon < ConColor.YELLOW)
                {
                    for (int level = botToAward.Level - 1; level > 1; level--)
                    {
                        if (ConLevels.GetConColor(ConLevels.GetConLevel(botToAward.Level, level)) == highestCon)
                        {
                ...
                else if (highestCon > ConColor.YELLOW)
                {
                    equivalentLevel = botToAward.Level + 1;
                    for (int level = equivalentLevel; level < 51; level++)
                    {
                        ConColor color = ConLevels.GetConColor(ConLevels.GetConLevel(botToAward.Level, level));
                        if (color == highestCon)
                            equivalentLevel = level;
                        else if (color > highestCon)
                            break;
                    }
                }
                else
                {
                    equivalentLevel = botToAward.Level;
                }

                return (long)Math.Ceiling(
                    (double)killedNpc.GetExperienceValueForLevel(equivalentLevel) / memberCount);
            }
        }
