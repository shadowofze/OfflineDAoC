...
namespace DOL.GS.ServerRules
{
	...
	public class NormalServerRules : AbstractServerRules
	{
		...
		{
			if (BotPvpCrowdControl.Protected(attacker, defender)) return false;
			...
			{
				if (attackerBrain.GetLivingOwner() is GameLiving livingOwner)
			...
			if (defender is GameNPC npcDefender)
			{
				if (npcDefender.Brain is IControlledBrain defenderBrain
					&& defenderBrain.GetLivingOwner() is GameLiving livingOwner)
					defender = livingOwner;
			}
		...
		public override bool IsSameRealm(GameLiving source, GameLiving target, bool quiet)
		{
			...
			if (source is GameNPC)
			{
				...
				if (controlled?.GetLivingOwner() is GameLiving livingOwner)
				{
					source = livingOwner;
