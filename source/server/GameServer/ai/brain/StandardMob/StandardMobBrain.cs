    ...
    public class StandardMobBrain : APlayerVicinityBrain, IOldAggressiveBrain
    {
        ...
        protected virtual void CheckNpcAggro()
        {
            foreach (GameNPC npc in Body.GetNPCsInRadius((ushort) AggroRange))
            {
                ...
                // GameBots occupy the NPC collection, but are player-like for
                // natural aggro. Apply the same exclusions as CheckPlayerAggro.
                if (npc is GameBot bot &&
                    (bot.IsStealthed || bot.IsOnStableMasterRoute ||
                     bot.effectListComponent.ContainsEffectForEffectType(eEffect.Shade)))
                    continue;

                if (Properties.CHECK_LOS_BEFORE_AGGRO)
                {
                    // Bots have no client packet connection to answer the normal
                    // LoS callback. On navmeshed ground use the server geometry,
                    // preventing natural aggro through walls while retaining the
                    // legacy fallback in zones without a mesh.
                    if ((npc is GameBot || IsGameBotOwnedNecromancerServant(npc)) &&
                        PathfindingProvider.Instance.IsAvailable &&
                        PathfindingProvider.Instance.HasNavmesh(Body.CurrentZone))
                    {
                        Vector3 source = new(Body.X, Body.Y, Body.Z);
                        Vector3 destination = new(npc.X, npc.Y, npc.Z);
                        if (!PathfindingProvider.Instance.HasLineOfSight(
                                Body.CurrentZone, source, destination, PathfindingProvider.Instance.DefaultFilters))
                            continue;
                    }
        ...
        public virtual bool CanAggroTarget(GameLiving target)
        {
            ...
            // Evaluate player-owned pets against their player as before. A
            // GameBot Necromancer is the one deliberate exception: its shade
            // cannot be attacked while the servant lives, so the servant must
            // remain a natural PvE aggro target just like a real player's.
            GameLiving realTarget = NaturalAggroIdentity(target);
            ...
            return (Body.Realm != eRealm.None || realTarget is not GameNPC || IsPlayerLikeNaturalAggroTarget(realTarget)) && AggroLevel > 0;
        }

        /// <summary>
        /// GameBots live in the NPC spatial collection for scalability, but are
        /// player characters for natural PvE aggro eligibility.
        /// </summary>
        public static bool IsPlayerLikeNaturalAggroTarget(GameLiving target) =>
            target != null && IsPlayerLikeNaturalAggroType(target.GetType());

        public static bool IsPlayerLikeNaturalAggroType(Type targetType) =>
            targetType != null &&
            (typeof(GamePlayer).IsAssignableFrom(targetType) || typeof(GameBot).IsAssignableFrom(targetType));

        public static GameLiving NaturalAggroIdentity(GameLiving target)
        {
            if (target is not GameNPC npcTarget || npcTarget.Brain is not IControlledBrain controlled)
                return target;

            GameLiving owner = npcTarget is NecromancerPet
                ? controlled.GetLivingOwner()
                : controlled.GetPlayerOwner();
            return owner ?? target;
        }

        private static bool IsGameBotOwnedNecromancerServant(GameNPC target) =>
            target is NecromancerPet && target.Brain is IControlledBrain controlled &&
            controlled.GetLivingOwner() is GameBot;
        ...
        private bool CheckOffensiveSpells(List<Spell> spells)
        {
            ...
            return Body.CastSpell(spellToCast, m_mobSpellLine,
                checkLos: ShouldCheckLosForOffensiveSpell(spellToCast, Body.TargetObject as GameLiving));
        }

        /// <summary>
        /// Extension point for controlled NPCs that must remain autonomous when
        /// no player client is available to proxy an offensive LoS request.
        /// Ordinary NPC and pet behavior keeps the original client-assisted LoS.
        /// </summary>
        protected virtual bool ShouldCheckLosForOffensiveSpell(Spell spell, GameLiving target) => true;
        ...
        private bool CheckDefensiveSpells(List<Spell> spells)
        {
            ...
            bool cast = Body.CastSpell(candidate.spell, m_mobSpellLine,
                ShouldCheckLosForDefensiveSpell(candidate.spell, candidate.target));
            Body.TargetObject = oldTarget;
            return cast;
        }

        /// <summary>
        /// Extension point for controlled NPC support brains. Ordinary NPCs
        /// retain the original client-assisted line-of-sight check.
        /// </summary>
        protected virtual bool ShouldCheckLosForDefensiveSpell(Spell spell, GameLiving target) => true;
