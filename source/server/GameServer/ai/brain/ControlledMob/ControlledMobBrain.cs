	...
	public class ControlledMobBrain : StandardMobBrain, IControlledBrain
	{
        ...
        public virtual GamePlayer GetPlayerOwner()
        {
            ...
            while (owner is GameNPC && owner != null)
            {
                ...
                // A persistent GameBot is the root living for its pets and has
                // no human Owner. Temporary group helpers still resolve to the
                // real player that created them.
        ...
        public virtual GameNPC GetNPCOwner()
        {
            ...
            while (owner != null)
            {
                ...
                if (owner is GameBot botOwner)
                    return botOwner.Owner == null ? botOwner : null;
		...
		{
            if (!CompanionEngagementMode.Allows(Body, target as GameLiving)) return;
		...
		public override void CheckAbilities()
		{
			...
			foreach (Ability ab in Body.Abilities.Values)
			{
				switch (ab.KeyName)
				{
					...
					{
						GameLiving protectedOwner = GetInterceptOwner();

						if (protectedOwner != null)
						{
							InterceptAbilityHandler.CheckExistingEffectsOnTarget(Body, protectedOwner, false, out bool foundOurEffect, out InterceptECSGameEffect existingEffectFromAnotherSource);
							...
							ECSGameEffectFactory.Create(new(Body, 0, 1, null), Body, protectedOwner, static (in i, body, owner) => new InterceptECSGameEffect(i, body, owner));
						}
		...
		/// <summary>
		/// Spirit pets intercept for their immediate player-like master. A temporary
		/// companion bot has a real-player creator above it, but that creator is not
		/// the Spiritmaster who summoned and controls this pet.
		/// </summary>
		protected virtual GameLiving GetInterceptOwner()
		{
			return Owner is GameBot ? Owner : GetPlayerOwner();
		}
		...
		{
			if (m_orderAttackTarget != null && !CompanionEngagementMode.Allows(Body, m_orderAttackTarget))
		...
		public override void AttackMostWanted()
		{
			...
			if (owner_npc != null && owner_npc.Brain is StandardMobBrain)
			{
				if ((owner_npc.IsCasting || owner_npc.IsAttacking) &&
					...
					CompanionEngagementMode.Allows(Body, owner_npc.TargetObject as GameLiving) &&
		...
		public virtual void OnOwnerAttacked(AttackData ad)
		{
			...
			eCharacterClass? ownerClass = Owner switch
			{
				GameBot botOwner => (eCharacterClass)botOwner.CharacterClass.ID,
				GamePlayer playerOwner => (eCharacterClass)playerOwner.CharacterClass.ID,
				_ => GetLivingOwner() switch
				{
					GameBot rootBot => (eCharacterClass)rootBot.CharacterClass.ID,
					GamePlayer rootPlayer => (eCharacterClass)rootPlayer.CharacterClass.ID,
					_ => null
				}
			};
			if (ownerClass.HasValue && !PetMayDefendOwner(ownerClass.Value))
				return;
			...
			// AddToAggroList changes state and wakes a newly-idle pet. Also wake
			// an already-engaged pet so a second attacker is considered without
			// waiting for its ordinary background interval.
			if (HasAggro)
				NextThinkTick = GameLoop.GameLoopTime;
		}

		/// <summary>
		/// Controlled pets defend both real-player and GameBot owners. Theurgist
		/// elementals retain their classic one-target fire-and-forget behavior.
		/// This policy does not enable autonomous proximity pulling.
		/// </summary>
		public static bool PetMayDefendOwner(eCharacterClass ownerClass) =>
			ownerClass is not eCharacterClass.Theurgist;
