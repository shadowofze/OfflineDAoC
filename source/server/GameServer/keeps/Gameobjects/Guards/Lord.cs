...
namespace DOL.GS.Keeps
{
    public class GuardLord : GameKeepGuard
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            ...
            if (source is GamePlayer or GameBot)
            {
                attacker = source as GameLiving;
            }
            else if (source is GameNPC { Brain: IControlledBrain controlled } && controlled.GetLivingOwner() is GameLiving owner)
            {
                attacker = owner;
            }
            ...
            {
                if (attacker != null)
                    (attacker as IGamePlayer)?.Out.SendMessage(this.Name + " is immune to damage from this range", eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
                return;
            }
