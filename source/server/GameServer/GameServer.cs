...
namespace DOL.GS
{
	...
	public class GameServer : BaseServer
	{
		...
		public override bool Start()
		{
			...
			try
			{
				AutonomousBotGoalPolicy.Initialize(Configuration.RootDirectory);
		...
		protected void SaveTimerProc(object sender)
		{
			...
			try
			{
				...
				(int count, long elapsed) autonomousBots = (0, 0);
				...
				if (m_database != null)
				{
					...
					Save(AutonomousBotRegistry.SaveAll, ref autonomousBots);
				}
				...
				if (log.IsInfoEnabled)
				{
					...
					stringBuilder.Append($"{nameof(autonomousBots)}: {autonomousBots.count} in {autonomousBots.elapsed}ms");
