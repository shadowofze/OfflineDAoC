namespace DOL.GS.PropertyCalc
{
    ...
    public class MaxConcentrationCalculator : PropertyCalculator
    {
        ...
        public override int CalcValue(GameLiving living, eProperty property) 
        {
            ...
            else if (living is GameBot bot)
            {
                if (bot.CharacterClass?.ManaStat is null or eStat.UNDEFINED)
                    return 1000000;
                int concBase = (int)(bot.Level * 4 * 2.2);
                int stat = bot.GetModified((eProperty)bot.CharacterClass.ManaStat);
                int conc = (concBase + (int)((stat - 50) * 2.8)) / 2;
                conc = System.Math.Max(0, (int)(bot.Effectiveness * conc));
                if (bot.MLLevel >= 4 && bot.GetSpellLine("Perfecter") != null)
                    conc += 20 * conc / 100;
                return conc;
            }
            else
