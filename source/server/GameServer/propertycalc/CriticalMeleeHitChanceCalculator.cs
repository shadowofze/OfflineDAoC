    ...
    public class CriticalMeleeHitChanceCalculator : PropertyCalculator
    {
        public override int CalcValue(GameLiving living, eProperty property)
        {
            ...
            if (living is GamePlayer)
                chance += 10;
            else if (living is NecromancerPet { Owner: GameBot botOwner })
            {
                // A companion's servant inherits from the companion, not its
                // human group leader; persistent bots have no GamePlayer owner.
                chance += 10;
                chance += botOwner.GetAbility<RealmAbilities.AtlasOF_WildMinionAbility>()?.Amount ?? 0;
            }
