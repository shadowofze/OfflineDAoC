...
namespace DOL.GS.PacketHandler
{
	...
	public class PacketLib168 : AbstractPacketLib, IPacketLib
	{
		...
		public virtual void SendObjectUpdate(GameObject obj, bool udp = true)
		{
			...
			// Keep the NPC health/target update, but make it inseparable in TCP
			// ordering from its riding association. A delayed UDP rider-position
			// packet can otherwise undo a newer mount packet and leave a standing,
			// snapping bot beside the horse. Taxi motion and real players are unchanged.
			GameBot stableRider = obj as GameBot;
			if (stableRider?.StableRouteMountForClient != null)
			{
				stableRider.SendStableRideAssociation(m_gameClient.Player);
				udp = false;
			}
			else
				stableRider = null;
			...
			stableRider?.SendStableRideAssociation(m_gameClient.Player, true);
        ...
        public virtual void SendObserverFlightMode(bool flying, bool debugEnabled)
		{
            using (var pak = PooledObjectFactory.GetForTick<GSTCPPacketOut>().Init(GetPacketCode(eServerPackets.DebugMode)))
            {
                bool gm = m_gameClient.Account.PrivLevel > 1;
                pak.WriteByte((byte)(gm && debugEnabled ? 1 : 0));
                // Offline client extension: activate/deactivate native flying state.
                // Ordinary DebugMode packets keep the reserved byte at zero.
                pak.WriteByte((byte)(gm && flying ? 1 : 2));
                SendTCP(pak);
            }
		}
		...
		{
            // Keep observer flight invisible through subsequent model refreshes,
            // without changing or saving the character's actual model.
            GamePlayer observer = m_gameClient.Player;
            if (observer != null && objectId == observer.ObjectID &&
                observer.TempProperties.GetProperty<bool>(Commands.FlyCommandHandler.ActiveKey))
            {
                newModel = Commands.FlyCommandHandler.InvisibleModel;
            }
		...
		public virtual void SendNPCCreate(GameNPC npc)
		{
			...
			using (var pak = PooledObjectFactory.GetForTick<GSTCPPacketOut>().Init(GetPacketCode(eServerPackets.NPCCreate)))
			{
				...
				if (NecromancerPetAppearance.HasEquipment(npc))
		...
		public virtual void SendLivingEquipmentUpdate(GameLiving living)
		{
			...
			using (var pak = PooledObjectFactory.GetForTick<GSTCPPacketOut>().Init(GetPacketCode(eServerPackets.EquipmentUpdate)))
			{
				...
				pak.WriteByte(NecromancerPetAppearance.WeaponSlots(living));

				if (NecromancerPetAppearance.HasEquipment(living))
				{
					var items = NecromancerPetAppearance.Equipment(living);
		...
		public virtual void SendGroupWindowUpdate()
		{
			...
			using (var pak = PooledObjectFactory.GetForTick<GSTCPPacketOut>().Init(GetPacketCode(eServerPackets.VariousUpdate)))
			{
				...
				{
                    pak.WriteByte((byte)(group.IsCompanionRaid ? Math.Min(8, (int)group.MemberCount) : group.MemberCount));
				}
				...
				if (group != null)
				{
                    ...
                    {
                        // The native raid extension has separate state. Never
                        // overflow the legacy client's eight-member arrays.
                        if (group.IsCompanionRaid && updateLiving.GroupIndex >= 8)
                            continue;
		...
		public virtual void SendMinotaurRelicMapRemove(uint id)
		{
		}

		public virtual void SendMinotaurRelicMapUpdate(uint id, ushort region, int x, int y, int z)
		{
		}
        ...
        {
            if (living.Group?.IsCompanionRaid == true && living.GroupIndex >= 8)
                return;
