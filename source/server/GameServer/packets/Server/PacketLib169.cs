	...
	public class PacketLib169 : PacketLib168
	{
		...
		public override void SendGroupWindowUpdate()
		{
			...
			using (var pak = PooledObjectFactory.GetForTick<GSTCPPacketOut>().Init(GetPacketCode(eServerPackets.VariousUpdate)))
			{
				...
				{
					pak.WriteByte((byte)(group.IsCompanionRaid ? System.Math.Min(8, (int)group.MemberCount) : group.MemberCount));
				}
				...
				if (group != null)
				{
					...
					{
						if (group.IsCompanionRaid && living.GroupIndex >= 8) continue;
		...
		{
			if (living.Group?.IsCompanionRaid == true && living.GroupIndex >= 8) return;
