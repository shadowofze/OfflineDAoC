    ...
    public class PlayerPositionUpdateHandler : PacketHandler
    {
        ...
        private static void Handle(GameClient client, GSPacketIn packet)
        {
            ...
            if (client.Player.Steed != null && client.Player.Steed.ObjectState is GameObject.eObjectState.Active)
            {
                ...
                bool playerTaxi = PlayerTaxiSynchronization.IsPlayerTaxi(rider) && client.Version >= GameClient.eClientVersion.Version1124;
                bool distant = !rider.IsWithinRadius(steed, 1000);
                if (playerTaxi)
                {
                    // Compare the newly received position, not the previous packet's
                    // position. The latter falls behind during a server/client pause.
                    long savedPosition = packet.Position;
                    try
                    {
                        float x = packet.ReadFloatLowEndian(), y = packet.ReadFloatLowEndian(), z = packet.ReadFloatLowEndian();
                        distant = PlayerTaxiSynchronization.NeedsCorrection(x, y, z, steed.X, steed.Y, steed.Z);
                    }
                    finally { packet.Position = savedPosition; }
                }
                if (distant)
                {
                    if (playerTaxi)
                    {
                        PlayerTaxiSynchronization.Correct(rider, true);
                        return;
                    }
                    rider.X = steed.X;
                    rider.Y = steed.Y;
        ...
        public static bool ProcessStateFlags(GamePlayer player, StateFlags stateFlags)
        {
            ...
            if (PlayerTaxiSynchronization.IsPlayerTaxi(player) && (stateFlags & StateFlags.RIDING) == 0)
                PlayerTaxiSynchronization.Correct(player, false);
