...
namespace DOL.GS
{
	...
	public abstract class GameLiving : GameObject, IPooledList<GameLiving>
	{
        ...
        public virtual int MeleeAttackRange => 200 + DragonCombatGeometry.TargetReach(TargetObject);
...
;
		...
		public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
		{
			...
			if (source is GameNPC npcSource
				&& npcSource.Brain is IControlledBrain brain
				&& brain.GetLivingOwner() is GameLiving livingOwner)
			{
				source = livingOwner;
			}
			...
			{
				if (source is GamePlayer attackerPlayer)
				{
					// Apply Mauler RA5L
		...
		public virtual void OnAttackEnemy(AttackData ad)
		{
			...
			PlayerLedPullCoordinator.OnAttack(this, ad);
			AutonomousDefensivePull.OnAttack(this, ad);
		...
		public virtual int GetModified(eProperty property)
		{
			...
			if (property == eProperty.Undefined)
				return 0;
			if (m_propertyCalc != null && propertyId >= 0 && propertyId < m_propertyCalc.Length && m_propertyCalc[propertyId] != null)
			{
		...
		{
			// Undefined is a deliberate sentinel used by spell lines without a
			// specialization. It contributes no item bonus and is not an error.
			if (property == eProperty.Undefined)
				return 0;
			int propertyId = (int)property;
			if (m_propertyCalc != null && propertyId >= 0 && propertyId < m_propertyCalc.Length && m_propertyCalc[propertyId] != null)
