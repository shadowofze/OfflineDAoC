...
namespace DOL.GS
{
    ...
    {
        // Set only at an explicit cleanup boundary; no polling or DB writes.
        internal string BotReleaseReason { get; set; }
        public int AppearanceTemplateId { get; private set; }
        ...
        {
            AppearanceTemplateId = npcTemplate.TemplateId;
            // Update max health on summon.
            GetModified(eProperty.MaxHealth);
            // Set immunities/load equipment/etc.
            // Keep the original level-45 equipment/combat path when its display name changes.
            switch (AppearanceTemplateId == 206 ? "abomination" : Name.ToLower())
            {
        ...
        public override int Health
        {
            ...
            set
            {
                ...
                if (oldPercent != HealthPercent)
                {
                    ...
                    if ((Brain as IControlledBrain)?.Owner is GamePlayer owner)
        ...
        // The legacy abomination equipment/spell rows are absent in this distribution.
        // Keep its existing melee damage unchanged: this innate proc does not equip a
        // real weapon or alter DPS/speed. The private sword remains presentation only.
        public const double HeroFireProcChance = 10.0 * (50.0 / 35.0) * 0.01;
        public const int HeroFireProcDamage = 95;
        private static readonly Spell HeroFireProc = new(new DbSpell
        {
            AllowAdd = false,
            SpellID = 32053,
            Name = "Abomination Fire Strike",
            Type = eSpellType.DirectDamage.ToString(),
            Target = eSpellTarget.ENEMY.ToString(),
            DamageType = (int)eDamageType.Heat,
            Damage = HeroFireProcDamage,
            ClientEffect = 2303,
            Icon = 2303,
            Range = 1500,
            CastTime = 0,
        ...
        public bool CanTriggerHeroFireProc(AttackData ad) =>
            AppearanceTemplateId == 206 && ad != null && ad.Attacker == this &&
            ad.IsMeleeAttack && ad.AttackResult is eAttackResult.HitStyle or eAttackResult.HitUnstyled &&
            ad.Target != null && ad.Target.IsAlive;
        ...
        public override void CheckWeaponMagicalEffect(AttackData ad)
        {
            ...
            if (CanTriggerHeroFireProc(ad) && Chance(RandomDeckEvent.OffensiveProcChance, HeroFireProcChance))
                ApplyHeroFireProc(ad);
        }

        protected virtual void ApplyHeroFireProc(AttackData ad)
        {
            // Same spell pipeline as ordinary weapon procs: resist, damage, aggro and
            // kill credit remain normal. Never replace the pet's queued owner spell.
            var line = SkillBase.GetSpellLine(GlobalSpellsLines.Item_Effects);
            ScriptMgr.CreateSpellHandler(this, HeroFireProc, line)?.StartSpell(ad.Target);
        }
        ...
        {
            if ((Brain as IControlledBrain)?.Owner is not (GamePlayer or IGamePlayer))
                return;

            if (Owner is GameBot)
                BotReleaseReason = "tether";
            Brain.Stop();
            Die(null);
        }
