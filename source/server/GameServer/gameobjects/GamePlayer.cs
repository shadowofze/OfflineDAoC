...
namespace DOL.GS
{
    ...
    public class GamePlayer : GameLiving, IGameStaticItemOwner, IPooledList<GamePlayer>
    {
        ...
        public override void OnAttackedByEnemy(AttackData ad)
        {
            ...
            // Companions assist only when they are nearby members of this exact
            // group. Solo bots and unrelated groups are intentionally excluded.
        ...
        public override int MeleeAttackRange
        {
            get
            {
                ...
                return range + DragonCombatGeometry.TargetReach(TargetObject);
            }
        }
        ...
        public override bool Say(string str)
        {
            ...
            AutonomousBotChatCoordinator.OnPlayerLocalChat(this, str);
            return true;
        }
        ...
        {
            bool changingRegion = regionID != CurrentRegionID;
            ...
            if (regionID != CurrentRegionID)
            {
                ...
                // A cross-region MoveTo completes after the client loads the
                // destination.  Record it now; the helper event/failsafe will
                // consume the marker only once this player is active there.
            ...
            if (DragonCombatGeometry.IsDisplacing(this))
                return true;
            TemporaryGroupStableTravel.MarkAcceptedPlayerTransfer(this);
            TemporaryGroupStableTravel.RelocateHelpersAfterPlayerTransfer(this, explicitMoveTo: true);
        ...
        {
            get { return TempProperties.GetProperty<bool>(DOL.GS.Commands.FlyCommandHandler.ActiveKey) || m_invulnerabilityTick > CurrentRegion.Time; }
        }
