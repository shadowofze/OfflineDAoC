		...
		public virtual void SetControlledBrain(IControlledBrain controlledBrain)
		{
			...
			else
			{
				...
				if (controlledBrain.Body != null)
				{
					...
					// Cosmetic pet equipment also sets the create packet's wait-for-equipment flag.
					// Always complete that handshake when the owner pet window recreates the NPC.
					if (NecromancerPetAppearance.HasEquipment(controlledBrain.Body))
