			...
			if (source is GamePlayer)
			{
				...
				if (item.Item_Type == 40 && isItemInMerchantList(item))
				{
					...
					if ((path != null) && ((Math.Abs(path.X - this.X)) < 500) && ((Math.Abs(path.Y - this.Y)) < 500))
					{
						...
						// The real player's ticket was just consumed by the native stable
						// master.  Only that player's in-range /spawn companions may mirror
						// this exact path; they neither own nor buy tickets themselves.
						TemporaryGroupStableTravel.QueueMirroredStableRoute(player, path, item.Template);
						return true;
					}
				}
				else
