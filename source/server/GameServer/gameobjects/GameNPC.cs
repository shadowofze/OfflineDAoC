...
namespace DOL.GS
{
	...
	public class GameNPC : GameLiving, ITranslatableObject, IPooledList<GameNPC>
	{
		...
		{
			// GameBots are player characters, even when Level/LoadTemplate enters
			// through the NPC lifecycle. All other NPC types keep the original path.
		...
		public override eRealm Realm
		{
			get
			{
				IControlledBrain brain = Brain as IControlledBrain;
				if (brain?.Owner != null)
					return brain.Owner.Realm; // controlled pets always use their owner's realm
				return base.Realm;
			}
			set
			{
		...
		protected long m_lastVisibleToAutonomousBotTick = -VISIBLE_TO_PLAYER_SPAN;
		...
		/// <summary>
		/// True while this NPC is close enough to a real player or an autonomous
		/// player-like bot to need its normal vicinity brain. This is deliberately
		/// separate from IsVisibleToPlayers: a server bot must wake gameplay AI but
		/// must not pretend that a client is present for packet/effect visibility.
		/// </summary>
		public virtual bool IsVisibleToPlayersOrBots =>
			IsVisibleToPlayers || GameLoop.GameLoopTime - m_lastVisibleToAutonomousBotTick < VISIBLE_TO_PLAYER_SPAN;
		...
		{
			movementComponent.ForcePathReplot();
		}

		public bool TryConsumeAutonomousPathFailure(out PathfindingStatus status, out Vector3 destination)
		{
			return movementComponent.TryConsumeAutonomousPathFailure(out status, out destination);
		}
		...
		/// <summary>
		/// Keeps a player-vicinity brain alive when an autonomous GameBot is a
		/// legitimate nearby gameplay target. The caller is responsible for using
		/// the brain's authored aggro range and CanAggroTarget rules.
		/// </summary>
		public virtual void OnAutonomousBotNearby()
		{
			m_lastVisibleToAutonomousBotTick = GameLoop.GameLoopTime;
		...
		{
			// Autonomous and temporary GameBots can carry a stale visual item in a
			// weapon slot (for example a focus staff on a Shadowblade).  The native
			// helper deliberately picks a two-hand slot at random, which is correct
			// for ordinary NPC templates but can select that invalid item for a bot.
			// Keep the original NPC behavior byte-for-byte outside this GameBot-only
			...
			if (this is GameBot bot)
			{
				...
				// off-hand-only loadout is not usable even when the left item is a
				// class-valid weapon; let GameBot reconciliation move/create a real
				// primary weapon before starting the swing cycle.
				bool standardUsable = BotWeaponStats.CanUseMelee(bot, rightHand);
				bool twoHandUsable = BotWeaponStats.CanUseMelee(bot, twoHand);
				bool preferTwoHand = twoHandUsable &&
				                    (!standardUsable || bot.BotSpec?.Is2H == true);

				if (!standardUsable && !twoHandUsable)
				{
					bot.EnsureBotWeaponReady();
					rightHand = Inventory?.GetItem(eInventorySlot.RightHandWeapon);
					twoHand = Inventory?.GetItem(eInventorySlot.TwoHandWeapon);
					standardUsable = BotWeaponStats.CanUseMelee(bot, rightHand);
					twoHandUsable = BotWeaponStats.CanUseMelee(bot, twoHand);
					preferTwoHand = twoHandUsable &&
					                    (!standardUsable || bot.BotSpec?.Is2H == true);
				}
				...
				eActiveWeaponSlot newBotSlot = preferTwoHand
					? eActiveWeaponSlot.TwoHanded
					: eActiveWeaponSlot.Standard;
				if (newBotSlot != ActiveWeaponSlot)
				{
					if (attackComponent.AttackState)
						attackComponent.StopAttack();
					SwitchWeapon(newBotSlot);
				}
				StartAttack(target);
				return;
			}
		...
		{
			if (this is GameBot bot && !BotRangedCombat.CanUse(bot, Inventory?.GetItem(eInventorySlot.DistanceWeapon)))
			{
				// Repair/create the bot-only distance starter without changing a
				// human or ordinary NPC inventory.  If no legal ranged weapon can be
				// made, leave the caller on its existing melee decision instead of
				// entering a zero-damage draw loop.
				BotRangedCombat.EnsureStarter(bot);
				if (!BotRangedCombat.CanUse(bot, Inventory?.GetItem(eInventorySlot.DistanceWeapon)))
					return;
			}
		...
		/// </summary>
		protected virtual bool RetainCorpseForResurrection => false;
		protected virtual bool RetainGroupWhenDead => false;
		...
		public override void ProcessDeath(GameObject killer)
		{
			...
			// Resolve a real controlled pet to its root owner, but preserve an
			// ownerless persistent GameBot as the killer. BotBrain implements the
			// controlled-brain contract for companion commands; treating every such
			// NPC as a pet turned autonomous kills into killer=null and skipped the
			// complete XP/loot reward path.
			if (killer is GameNPC pet && pet.Brain is IControlledBrain petBrain &&
				petBrain.GetLivingOwner() is GameLiving livingOwner)
			{
				killer = livingOwner;
			}
			...
			if (!RetainCorpseForResurrection)
			{
				Delete();
		...
		/// </summary>
		// Read-only status for event dashboards; never changes the native timer.
		public long RespawnRemainingMilliseconds => IsRespawning ? Math.Max(0, m_respawnTimer.TimeUntilElapsed) : 0;
		...
		public override long LastAttackTickPvE
		{
			set
			{
				...
				// Independent playerbots deliberately implement IControlledBrain so
				// they can reuse the normal combat FSM, but unlike pets they have no
				// owner to mirror combat timestamps to.
				if (Brain is IControlledBrain { Owner: GameLiving owner })
					owner.LastAttackTickPvE = value;
			}
		}
		...
		public override long LastAttackedByEnemyTickPvE
		{
			set
			{
				...
				if (Brain is IControlledBrain { Owner: GameLiving owner })
					owner.LastAttackedByEnemyTickPvE = value;
			}
		}
		...
		public override long LastAttackedByEnemyTickPvP
		{
			set
			{
				...
				if (Brain is IControlledBrain { Owner: GameLiving owner })
					owner.LastAttackedByEnemyTickPvP = value;
			}
		}
