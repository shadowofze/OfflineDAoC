	...
	public class GameSiegeCatapult : GameSiegeWeapon
	{
		...
		public override void DoDamage()
		{
			...
			foreach (GameLiving living in targets)
			{
				...
				{
					ad.Attacker = Owner;
					int before = living.Health;
					living.TakeDamage(Owner, eDamageType.Crush, damageAmount, 0);
					RecordBotHit(before, living);
					living.OnAttackedByEnemy(ad);
	
					Owner.OnAttackEnemy(ad);
					OwnerOut?.SendMessage("The " + this.Name + " hits " + living.Name + " for " + damageAmount + " damage!", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
				}
				else
				{
					ad.Attacker = this;
