...
namespace DOL.GS.Styles
{
	...
	public class StyleProcessor
	{
		...
		public static bool ExecuteStyle(AttackData attackData, double unstyledDamage, double unstyledDamageCap, out double styleDamage, out double styleDamageCap, out int animationId)
		{
            ...
            else if (weapon != null && living is GameBot bot)
                ApplyEnduranceCost(bot, weapon, style, false);
		...
		private static int CalculateEnduranceCost(GameLiving living, Style style, int weaponSpd, bool allowFreeProc = true)
		{
			...
			if (allowFreeProc && ValhallasBlessing != null && Util.Chance(75))
				return 0;
        ...
        public static bool CheckEnduranceCost(GameBot bot, DbInventoryItem weapon, Style style) =>
            weapon != null && style != null && bot.Endurance >= CalculateEnduranceCost(bot, style, weapon.SPD_ABS, false);

        public static bool ApplyEnduranceCost(GameBot bot, DbInventoryItem weapon, Style style, bool missed)
        {
            int cost = CalculateEnduranceCost(bot, style, weapon.SPD_ABS);
            bot.Endurance -= missed ? cost / 2 : cost;
            return true;
        }
		...
		protected static ISpellHandler CreateMagicEffect(GameLiving caster, GameLiving target, int spellID)
		{
			...
			// Scale true NPC procs here, since they cannot be scaled on
			// initialization. GameBot implements the player contract and must use
			// the authored player style proc unchanged. The old call also discarded
			// GetScaledSpell's copied result, so real NPC scaling was never applied.
			if (caster is GameNPC npc && caster is not IGamePlayer)
