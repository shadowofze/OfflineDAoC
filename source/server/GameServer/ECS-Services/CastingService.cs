    ...
    public sealed class CastingService : GameServiceBase
    {
        ...
        private static void TickInternal(CastingComponent castingComponent)
        {
            ...
            catch (Exception e)
            {
                ...
                // A spell handler exception must never leave IsCasting latched
                // forever.  The service still records the full exception above,
                // but clears the failed current/queued handlers so an NPC or bot
                // can resume movement and make a fresh decision on its next turn.
                castingComponent.ClearSpellHandlers();
            }
        }
    }
}
