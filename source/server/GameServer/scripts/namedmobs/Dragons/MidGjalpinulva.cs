...
namespace DOL.GS
{
	public class MidGjalpinulva : GameEpicBoss
	{
		...
		public override void LoadFromDatabase(DataObject obj)
		{
			...
			DragonLairPlacement.Restore(this, eRealm.Midgard);
		...
		{
			RealmEventPolicy.CapDragonDamage(ad);
			base.DealDamage(ad);
		}

		public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
		{
			if (DragonLairPlacement.CanReceiveDamageFrom(source))
			{
		...
		{
            if (Brain is MidGjalpinulvaBrain encounterBrain) encounterBrain.EncounterAdds.Reset();
				// debug
				if (killer == null)
		...
		public override int StickMinimumRange => 300;
		...
		private static readonly Point3D spawnPoint = DragonLairPlacement.Home(eRealm.Midgard);
...
namespace DOL.AI.Brain
{
	public class MidGjalpinulvaBrain : StandardMobBrain
	{
			...
			: base()
		{
			...
			_roamingPathPoints.Add(DragonLairPlacement.Home(eRealm.Midgard));//grounded lair
		}
		...
		public override void Think()
		{
			...
			if (Body.CurrentRegion.IsPM && Body.CurrentRegion.IsNightTime == false && !LockIsRestless && !HasAggro && !Body.InCombatInLast(30000) && _lastRoamIndex < _roamingPathPoints.Count)//Dragon will start roam
			{
			...
			if (!ResetChecks && _lastRoamIndex >= _roamingPathPoints.Count && DragonFlightLanding.HasArrived(Body, Body.SpawnPoint))
			{
            ...
            if (HasAggro && Body.TargetObject != null)
            {
				...
				// Combat teleports and throws are disabled for offline companion raids.
				...
				// Add summons are budgeted and staggered by EncounterAdds; no messenger waves.
			}
		...
		private void DragonFlyingPath()
        {
	        if (IsRestless && Body.IsAlive)
            {
				...
				{
					// Finish the airborne leg using 3D flight, not ground navmesh.
					// Keep IsRestless/FLYING until Think verifies actual touchdown.
					if (!Body.IsMoving && !DragonFlightLanding.HasArrived(Body, Body.SpawnPoint))
					{
						log.Info($"DRAGON_LANDING_APPROACH name=\"{Body.Name}\" position={Body.X},{Body.Y},{Body.Z} home={Body.SpawnPoint.X},{Body.SpawnPoint.Y},{Body.SpawnPoint.Z}");
						Body.WalkTo(Body.SpawnPoint, 400);
					}
				}
        ...
        {
            // Retired callback: even a stale timer cannot create another wave.
            return 0;
        }
		#endregion
		...
		private Spell Dragon_DD2
		{
			get
			{
				if (m_Dragon_DD2 == null)
				{
					...
					spell.Damage = RealmEventPolicy.DragonAttackDamageLimit;
...
namespace DOL.AI.Brain
{
	public class GjalpinulvaMessengerBrain : StandardMobBrain
	{
        ...
        {
            // Retired reinforcement chain. DragonEncounterAdds is the only summon owner.
        }
    }
}
...
namespace DOL.GS
{
	public class GjalpinulvaSpawnedAdd : GameNPC
	{
        ...
        public override bool AddToWorld()
		{
            ...
            // Lifetime belongs to this fight, not the legacy messenger list.
...
namespace DOL.AI.Brain
{
	public class GjalpinulvaSpawnedAdBrain : DragonEncounterAddBrain
	{
		private static readonly Logging.Logger log = Logging.LoggerManager.Create(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
