...
namespace DOL.GS
{
	public class HibCuuldurach : GameEpicBoss
	{
		...
		public override void LoadFromDatabase(DataObject obj)
		{
			...
			DragonLairPlacement.Restore(this, eRealm.Hibernia);
		...
		{
            if (Brain is HibCuuldurachBrain encounterBrain) encounterBrain.EncounterAdds.Reset();
				// debug
				if (killer == null)
		...
		private static readonly Point3D spawnPoint = DragonLairPlacement.Home(eRealm.Hibernia);
...
namespace DOL.AI.Brain
{
	public class HibCuuldurachBrain : StandardMobBrain
	{
			...
			: base()
		{
			...
			_roamingPathPoints.Add(DragonLairPlacement.Home(eRealm.Hibernia));//grounded Classic lair
		...
		public override void Think()
		{
			...
			if (Body.CurrentRegion.IsPM && Body.CurrentRegion.IsNightTime == false && !LockIsRestless && !HasAggro && !Body.InCombatInLast(30000))//Dragon will start roam
			{
...
namespace DOL.AI.Brain
{
	public class CuuldurachSpawnedAdBrain : DragonEncounterAddBrain
	{
		private static readonly Logging.Logger log = Logging.LoggerManager.Create(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
