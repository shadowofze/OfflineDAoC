...
namespace DOL.GS
{
    public class ApocInitializator : GameNPC
    {
        ...
        private long _encounterReadyAt;
        public long EncounterRespawnRemainingMilliseconds => Math.Max(0, _encounterReadyAt - GameLoop.GameLoopTime);
        ...
        {
            BroadcastMessage(String.Format("Fames asks, 'You, "+(RandomTarget?.Name ?? "adventurer")+", do you come to challenge fate? Come to me with your answer so that I may see the answer" +
        ...
        public int SpawnHorsemanFames(ECSGameTimer timer)
        {
            ...
            if (RandomTarget is GameBot)
            {
                // The chosen expedition representative accepts the same challenge
                // and receives the same two-minute preparation period as a player.
                Add.TryAcceptBotChallengeFrom(RandomTarget);
            }
        ...
        List<GameLiving> PlayersInRoom = new List<GameLiving>();
        public static bool PickedTarget = false;
        public void PlayerEnter()
        {
            PlayersInRoom.Clear();
            foreach (GameLiving player in EpicEncounterActors.Nearby(this, 1500))
            {
                if (player != null)
                {
                    if (EpicEncounterActors.CanTrigger(player))
                    {
            ...
            {
                GameLiving ptarget = PlayersInRoom[Util.Random(1, PlayersInRoom.Count) - 1];
        ...
        public void StartRespawnEncounter()
        {
            if (IsAlive)
            {
                if (!Fames.FamesIsUp && !Bellum.BellumUP && !Morbus.MorbusUP && !Funus.FunusUp && !Apocalypse.ApocUP && !start_respawn_check)
                {
                    ...
                    _encounterReadyAt = GameLoop.GameLoopTime + time;
                    ...
                    foreach (GamePlayer player in GetPlayersInRadius(1500))
                    {
                        if(player != null)
                        {
                            if(player.IsAlive)
                            {
                                ...
                                {
                                    if (EpicEncounterMechanics.AllowDisplacement) player.MoveTo(60, 29469, 25244, 19490, 2014);
...
namespace DOL.AI.Brain
{
    public class ApocIniBrain : StandardMobBrain
    {
        ...
        {
            // Encounter state, spawning and dialogue timers belong to the
            // region's normal NPC loop. An unowned System.Timer survived
            // removal and could mutate this encounter concurrently.
            if (Body is ApocInitializator initializer)
                initializer.DoStuff();
            base.Think();
        }
    }
...
namespace DOL.GS
{
    ...
    {
        public Fames() : base() { }
        private ECSGameTimer _challengeTimer;
        private int _challengeAccepted;
        internal bool AcceptChallenge()
        {
            if (CanInteract || !IsAlive || ObjectState != eObjectState.Active ||
                Interlocked.CompareExchange(ref _challengeAccepted, 1, 0) != 0)
                return false;
            // Set the guard before scheduling, not once per observing player.
            CanInteract = true;
            ApocInitializator.FamesWaitForText = false;
            _challengeTimer = new ECSGameTimer(this, new ECSGameTimer.ECSTimerCallback(StartFamesTimer), 120000);
            return true;
        }

        internal void TryAcceptBotChallenge()
        {
            if (CanInteract || !ApocInitializator.OthersCanInteract || !IsAlive)
                return;
            // The chosen human retains the native exclusive response window.
            // Once anybody may answer, a physically present expedition member
            // can accept instead of leaving an unattended dialogue deadlock.
            foreach (GameLiving actor in EpicEncounterActors.Nearby(this, 1500))
            {
                if (TryAcceptBotChallengeFrom(actor))
                    return;
            }
        }

        internal bool TryAcceptBotChallengeFrom(GameLiving actor)
        {
            // The dialogue takes several seconds. Its originally selected bot
            // may have died, left the room, or been removed in the meantime.
            return actor is GameBot && actor.ObjectState == eObjectState.Active &&
                EpicEncounterActors.CanTrigger(actor) && actor.CurrentRegionID == CurrentRegionID &&
                IsWithinRadius(actor, 1500) && AcceptChallenge();
        }
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            GameLiving player2 = ApocInitializator.RandomTarget;
            ...
            {            
                if (t == ApocInitializator.RandomTarget || ApocInitializator.OthersCanInteract == true)
                {
                    ...
                    switch (str.ToLower())
                    {
                        ...
                        case "yes":
                            {
                                if (!AcceptChallenge()) return false;
...
namespace DOL.AI.Brain
{
    public class FamesBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            if (Body is Fames challenge)
                challenge.TryAcceptBotChallenge();
...
namespace DOL.GS
{
    public class Morbus : GameEpicBoss
    {
        ...
        {
            if (EpicEncounterActors.IsCombatant(source))
            {
                if (EpicEncounterMechanics.AllowAddShield && Morbus_Swarm_count > 0)
                {
...
namespace DOL.GS
{
    public class Funus : GameEpicBoss
    {
        ...
        public override void TakeDamage(GameObject source, eDamageType damageType, int damageAmount, int criticalAmount)
        {
            // Offline raids cannot coordinate class/weapon damage puzzles or heal-on-hit rules.
            // Keep real damage, armor/resists, health, death phases, and the encounter tether.
            if (!EpicEncounterActors.IsCombatant(source)) return;
