    ...
    public class SpindlerBroodmotherBrain : StandardMobBrain
    {
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if (EpicEncounterMechanics.AllowDisplacement && Util.Chance(10))
                {
        ...
        List<GameLiving> Enemys_To_Mezz = new List<GameLiving>();
        public int PickRandomTarget(ECSGameTimer timer)
        {
            Enemys_To_Mezz.Clear();
            if (HasAggro && Body.IsAlive && Body.ObjectState == GameObject.eObjectState.Active)
            {
                Enemys_To_Mezz.AddRange(EpicEncounterActors.Nearby(Body, 1500));
                if (Enemys_To_Mezz.Count > 0)
                {
                    if (CanCast == false)
                    {
                        GameLiving Target = Enemys_To_Mezz[Util.Random(0, Enemys_To_Mezz.Count - 1)];//pick random target from list
            ...
            if (!CanCast) StartCastMezz = false;
            return 0;
        }
        public int CastMezz(ECSGameTimer timer)
        {
            if (HasAggro && Body.IsAlive && Body.ObjectState == GameObject.eObjectState.Active && RandomTarget != null)
            {
                GameLiving oldTarget = Body.TargetObject as GameLiving;//old target
                if (EpicEncounterActors.CanTrigger(RandomTarget) && RandomTarget.ObjectState == GameObject.eObjectState.Active &&
                    RandomTarget.CurrentRegionID == Body.CurrentRegionID && Body.IsWithinRadius(RandomTarget, 1500))
                {
        ...
        {
            if (!EpicEncounterMechanics.AllowDisplacement) return 0;
        ...
        public int TeleportPlayer(ECSGameTimer timer)
        {
            ...
            {
                if (EpicEncounterMechanics.AllowDisplacement) TeleportTarget.MoveTo(Body.CurrentRegionID, 21115, 53483, 11286, 2100);
