...
namespace DOL.AI.Brain
{
    public class IssordenBrain : StandardMobBrain
    {
        ...
        private ECSGameTimer _boltTimer;
        ...
        public override void Think()
        {
            ...
            if (HasAggro && Body.TargetObject != null)
            {
                ...
                if(!PrepareBolt)
                {
                    _boltTimer = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(PickPlayer), Util.Random(25000,35000));
                    PrepareBolt = true;
                }
            }
            base.Think();
        ...
        {
            if (HasAggro && Body.IsAlive && Body.ObjectState == GameObject.eObjectState.Active)
            {
                // Build a fresh local pool. The old player-only list retained
                // dead/departed targets forever and had no autonomous targets.
                var damage_enemies = new List<GameLiving>(EpicEncounterActors.Nearby(Body, 1800));
                if (damage_enemies.Count > 0)
                {
                    GameLiving Target = damage_enemies[Util.Random(0, damage_enemies.Count - 1)];
                    ...
                    _boltTimer = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(CastBolt), 2000);
                    return 0;
                }           
            }
            PrepareBolt = false;
        ...
        {
            if (HasAggro && Body.IsAlive && Body.ObjectState == GameObject.eObjectState.Active &&
                EpicEncounterActors.CanTrigger(RandomTarget) && RandomTarget.ObjectState == GameObject.eObjectState.Active &&
                RandomTarget.CurrentRegionID == Body.CurrentRegionID && Body.IsWithinRadius(RandomTarget, 1800))
            {
            ...
            _boltTimer = new ECSGameTimer(Body, new ECSGameTimer.ECSTimerCallback(ResetBolt), 3000);
            return 0;
        }
