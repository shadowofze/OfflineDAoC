...
namespace DOL.GS
{
    ...
    public class Region
    {
        ...
        // Exact live membership, maintained with the authoritative object table.
        // Siege supply searches must not scan every monster/pet/bot in a region.
        private GameMerchant[] _merchantObjects = Array.Empty<GameMerchant>();
        public IReadOnlyList<GameMerchant> Merchants => Volatile.Read(ref _merchantObjects);
        ...
        public virtual void LoadFromDatabase(DbMob[] mobObjs, ref long mobCount, ref long merchantCount, ref long itemCount, ref long bindCount)
        {
            ...
            BotReleaseBindPoints.Replace(this, bindPoints, PathfindingProvider.Instance);
        ...
        internal bool AddObject(GameObject obj)
        {
            ...
            lock (ObjectsSyncLock)
            {
                ...
                if (oidObj == null)
                {
                    ...
                    if (obj is GameMerchant merchant)
                    {
                        GameMerchant[] merchants = new GameMerchant[_merchantObjects.Length + 1];
                        Array.Copy(_merchantObjects, merchants, _merchantObjects.Length);
                        merchants[^1] = merchant;
                        Array.Sort(merchants, static (left, right) => left.ObjectID.CompareTo(right.ObjectID));
                        Volatile.Write(ref _merchantObjects, merchants);
                    }
        ...
        internal void RemoveObject(GameObject obj)
        {
            lock (ObjectsSyncLock)
            {
                ...
                if (obj is GameMerchant merchant)
                {
                    int merchantIndex = Array.IndexOf(_merchantObjects, merchant);
                    if (merchantIndex >= 0)
                    {
                        GameMerchant[] merchants = new GameMerchant[_merchantObjects.Length - 1];
                        Array.Copy(_merchantObjects, 0, merchants, 0, merchantIndex);
                        Array.Copy(_merchantObjects, merchantIndex + 1, merchants, merchantIndex, merchants.Length - merchantIndex);
                        Volatile.Write(ref _merchantObjects, merchants);
                    }
                }
