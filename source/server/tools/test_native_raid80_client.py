...
import build_native_raid80_client as build
...
assert len(allocations)==240 and len(set(allocations))==240
...
for i in range(80):packet(1,i,100-i,80-i,200+i,f'Member{i:02}')
for i in range(80):
    ...
    assert struct.unpack('<f',uc.mem_read(p+12,4))[0]==80-i
...
packet(2,80);assert windows[-1]==(0x77,1)
for i in range(80):
...
packet(3);assert windows[-2:]==[(0x76,0),(0x77,0)]
...
import build_native_raid_client as old
...
for i in range(80):
    label=root.find(f".//LabelDef[Adapter='raid_name{i}']")
    assert label is not None
    assert int(label.find('Position/Y').text)==25+(i%8)*27+(216 if i>=40 else 0)
    assert int(label.find('Position/X').text)==12+((i%40)//8)*122
packet(2,40)
setup();uc.mem_write(sp,struct.pack('<I',sentinel));uc.reg_write(UC_X86_REG_EAX,body)
uc.mem_write(body+8,struct.pack('<I',0x600+79))
uc.emu_start(report['blocks']['eventHandler']['address'],sentinel,count=10000)
assert len(selected)==80, 'Hidden lower rows must not select targets in 40-person mode'
packet(1,80,50,50,999,'OutOfRange')
assert read32(report['active'])==40, 'Out-of-range update overwrote adjacent state'
print('PASS: 240 native adapters; 80 live names/health/power/IDs and target clicks.')
print('PASS: packet bounds, 40/80 window selection, hide both, inactive and hidden-row click guards.')
print('PASS: 40-person layout byte-identical; 80 layout adds the matching 40 rows below.')
