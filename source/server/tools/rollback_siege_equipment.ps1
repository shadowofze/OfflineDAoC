...
foreach ($entry in $manifest.Entries) {
    Assert-Stopped
    $temporary = $entry.Target + '.siege-rollback'
...
Write-Output 'Siege equipment binaries restored. Accounts, inventories, coins, world database and settings were not rolled back.'
