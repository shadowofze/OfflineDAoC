...
$entries = foreach ($pair in $map) {
    ...
    if (Test-Path -LiteralPath ($target + '.realm-event-records-new')) { throw "Unexpected staging file: $target" }
...
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\realm-event-records-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
New-Item -ItemType Directory -Path $backup | Out-Null
$preserved = foreach ($relative in @('account.txt','data\opendaoc.sqlite3.db','data\opendaoc.sqlite3.db-wal','server\bot-goals.json')) {
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_realm_event_records.ps1') -Destination (Join-Path $backup 'ROLLBACK REALM EVENT RECORDS.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed while installing.' }
        $temporary = $entry.Target + '.realm-event-records-new'
    ...
    Assert-Stopped
    foreach ($entry in $entries) {
        Copy-Item -LiteralPath (Join-Path $backup $entry.Relative) -Destination ($entry.Target + '.realm-event-records-new') -Force
        [IO.File]::Move($entry.Target + '.realm-event-records-new', $entry.Target, $true)
    }
    throw
}
