...
$entries = foreach ($pair in $map) {
    ...
    if (Test-Path -LiteralPath ($target + '.raid-recruitment-new')) { throw "Unexpected staging file: $target" }
...
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\raid-recruitment-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_raid_recruitment.ps1') -Destination (Join-Path $backup 'ROLLBACK RAID RECRUITMENT.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed while installing.' }
        $temporary = $entry.Target + '.raid-recruitment-new'
    ...
    Assert-Stopped
    foreach ($entry in $entries) {
        Copy-Item -LiteralPath (Join-Path $backup $entry.Relative) -Destination ($entry.Target + '.raid-recruitment-new') -Force
        [IO.File]::Move($entry.Target + '.raid-recruitment-new', $entry.Target, $true)
    }
    throw
}
