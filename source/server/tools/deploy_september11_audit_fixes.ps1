...
Assert-Stopped
$backup = Join-Path $runtime ('rollback-september11-audit-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
New-Item -ItemType Directory -Path $backup | Out-Null
$entries = @()
foreach ($location in @('', 'lib')) {
    foreach ($name in @('GameServer.dll','GameServer.pdb','CoreDatabase.dll','CoreDatabase.pdb')) {
...
Write-Output "Installed and hash-verified GameServer and CoreDatabase DLL/PDB, root and lib. Rollback: $backup"
