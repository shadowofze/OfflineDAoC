...
Assert-Stopped
[xml]$tests = Get-Content -LiteralPath (Join-Path $sourceRoot 'build\ground-speed-test-results\ground-speed-final.trx') -Raw
if ([int]$tests.TestRun.ResultSummary.Counters.failed -ne 0 -or [int]$tests.TestRun.ResultSummary.Counters.passed -lt 1845) { throw 'Final regression tests have not passed.' }
[xml]$routes = Get-Content -LiteralPath (Join-Path $sourceRoot 'build\ground-speed-test-results\installed-keep-routes.trx') -Raw
...
foreach ($entry in $entries | Where-Object { $_.Relative.EndsWith('.dll') }) {
    if ($entry.OldHash -ne '2CC847BA975A725419D635545B68E618CCA4C3395AA10584733761CF1B4F722C') { throw 'Installed server differs from the inspected baseline.' }
}
if (!$Apply) { $entries | Format-Table Relative,OldHash,NewHash; return }
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\ground-speed-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_ground_speed.ps1') -Destination (Join-Path $backup 'ROLLBACK GROUND SPEED.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed during deployment.' }
        $temporary = $entry.Target + '.ground-speed-new'
