...
foreach ($entry in $manifest.Entries) {
    Assert-Stopped
    $temporary = $entry.Target + '.raid-speed-rollback'
...
Write-Output 'Pre-raid-speed-fix binaries restored. Accounts, inventories, coins, world database and settings were not rolled back.'
