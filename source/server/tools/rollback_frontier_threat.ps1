...
foreach ($entry in $manifest.Entries) {
    Assert-Stopped
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot $entry.Relative) -Destination ($entry.Target + '.frontier-threat-rollback')
    [IO.File]::Move($entry.Target + '.frontier-threat-rollback', $entry.Target, $true)
    if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Restored hash mismatch.' }
}
Write-Output 'Frontier threat binaries rolled back. Accounts, progress and Bot Goals Setting unchanged.'
