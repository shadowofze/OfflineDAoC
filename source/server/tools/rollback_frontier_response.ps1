...
foreach ($entry in $manifest.Entries) {
    Assert-Stopped
    $temporary = $entry.Target + '.frontier-response-rollback'
...
Write-Output 'Pre-frontier-response-fix binaries restored. Accounts, inventories, coins, world database and settings were not rolled back.'
