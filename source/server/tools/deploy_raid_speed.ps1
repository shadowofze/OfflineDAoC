...
Assert-Stopped
[xml]$tests = Get-Content -LiteralPath (Join-Path $sourceRoot 'build\raid-speed-test-results\raid-speed-final.trx') -Raw
if ([int]$tests.TestRun.ResultSummary.Counters.failed -ne 0 -or [int]$tests.TestRun.ResultSummary.Counters.passed -lt 1815) { throw 'Final regression tests have not passed.' }
...
foreach ($entry in $entries | Where-Object { $_.Relative.EndsWith('.dll') }) {
    if ($entry.OldHash -ne '31D8B6869FE271F403D4ECD7B2D2736E2744397C531FAA8557149C6D18E5448F') { throw 'Installed server differs from the inspected baseline.' }
}
if (!$Apply) { $entries | Format-Table Relative,OldHash,NewHash; return }
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\raid-speed-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_raid_speed.ps1') -Destination (Join-Path $backup 'ROLLBACK RAID SPEED.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed during deployment.' }
        $temporary = $entry.Target + '.raid-speed-new'
