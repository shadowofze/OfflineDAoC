...
$entries = foreach ($pair in $map) {
    ...
    if (Test-Path -LiteralPath ($target + '.realm-hub-staging-new')) { throw "Unexpected staging file: $target" }
...
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\realm-hub-staging-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_realm_hub_staging.ps1') -Destination (Join-Path $backup 'ROLLBACK REALM HUB STAGING.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed while installing.' }
        $temporary = $entry.Target + '.realm-hub-staging-new'
    ...
    Assert-Stopped
    foreach ($entry in $entries) {
        Copy-Item -LiteralPath (Join-Path $backup $entry.Relative) -Destination ($entry.Target + '.realm-hub-staging-new') -Force
        [IO.File]::Move($entry.Target + '.realm-hub-staging-new', $entry.Target, $true)
    }
    throw
}
