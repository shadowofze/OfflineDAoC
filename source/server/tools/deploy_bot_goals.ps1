...
$entries = foreach ($pair in $map) {
    ...
    if (Test-Path -LiteralPath ($target + '.bot-goals-new')) { throw "Unexpected staging file: $target" }
...
$backup = Join-Path (Split-Path $runtime -Parent) ('deployment-backups\bot-goals-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'rollback_bot_goals.ps1') -Destination (Join-Path $backup 'ROLLBACK BOT GOALS.ps1')
try {
    foreach ($entry in $entries) {
        Assert-Stopped
        if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Runtime changed while installing.' }
        $temporary = $entry.Target + '.bot-goals-new'
    ...
    Assert-Stopped
    foreach ($entry in $entries) {
        Copy-Item -LiteralPath (Join-Path $backup $entry.Relative) -Destination ($entry.Target + '.bot-goals-new') -Force
        [IO.File]::Move($entry.Target + '.bot-goals-new', $entry.Target, $true)
    }
    throw
}
