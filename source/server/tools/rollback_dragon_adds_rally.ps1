...
foreach ($entry in $manifest.Entries) {
    Assert-Stopped
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot $entry.Relative) -Destination ($entry.Target + '.dragon-adds-rollback')
    [IO.File]::Move($entry.Target + '.dragon-adds-rollback', $entry.Target, $true)
    if ((Get-FileHash -LiteralPath $entry.Target).Hash -ne $entry.OldHash) { throw 'Restored hash mismatch.' }
}
Write-Output 'Dragon adds and rally update rolled back. Accounts, progress and settings unchanged.'
