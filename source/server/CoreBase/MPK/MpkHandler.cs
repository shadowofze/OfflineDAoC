...
namespace DOL.MPK
{
	...
	public class MpkHandler
	{
		...
		public void Write(string fname)
		{
			...
			def.SetInput(nameBytes);
			def.Finish();

			// Deflate framing can be larger than a short archive name. The former
			// character-count buffer silently truncated names (and their zlib
			// stream), producing MPKs that extracted but were rejected by clients.
			name = new byte[nameBytes.Length + 64];
			def.Deflate(name);
			if (!def.IsFinished)
				throw new InvalidDataException("The MPK archive-name buffer was too small.");
