...
namespace DOL.Network
{
    public class BaseClient
    {
        ...
        /// <summary>
        /// Captured while the accepted socket is valid. Reading RemoteEndPoint
        /// later can throw ObjectDisposedException during a service tick; local
        /// timeout policy must never depend on a socket that may already be
        /// closing.
        /// </summary>
        public bool IsLoopbackConnection { get; }
        ...
        public BaseClient(Socket socket)
        {
            if (socket != null)
            {
                ...
                try
                {
                    IsLoopbackConnection = socket.RemoteEndPoint is IPEndPoint endpoint &&
